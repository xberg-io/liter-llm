...
sealed class LiterLlmError(message: String) : Exception(message) {
    ...
    data class IdempotencyConflict(val key: String) :
        LiterLlmError(
            "idempotency conflict: key '{key}' was already used with a different request body"
        )

    data class IdempotencyInFlight(val key: String) :
        LiterLlmError(
