...
object LiterLlm {
    ...
    fun encodeDataUrl(bytes: ByteArray, mime: String? = null): String =
        LiterLlmBridge.nativeEncodeDataUrl(
     ...
     *
     * The returned MIME string is extracted verbatim from the URL prefix — it is not validated or
     * normalised.
     */
     ...
     *
     * Performs an O(n) linear scan over the embedded registry (143 entries). Returns an owned value
     * so bindings can pass capability data without borrowing registry internals.
     *
     * For unknown `provider_name` values the function returns an all-`false` sentinel so callers
     * never need to handle `Option`.
     */
     ...
     */
    fun checkBound(context: String, currentLen: Long, incoming: Long, limit: Long): Unit =
        LiterLlmBridge.nativeCheckBound(context, currentLen, incoming, limit)

    /**
     * Install the `ring` crypto provider as the rustls process default, idempotently.
     *
     * rustls 0.23+ removed the implicit default provider. This function installs `ring` once per
     * process. Subsequent calls are no-ops. Calling it after another rustls crypto provider has
     * already been installed is safe: the `Err` from `install_default()` is silently ignored.
     *
