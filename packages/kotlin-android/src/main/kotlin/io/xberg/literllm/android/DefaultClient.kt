...
class DefaultClient internal constructor(internal val handle: Long) : AutoCloseable {
        ...
        return withContext(Dispatchers.IO) {
            val responseJson =
                LiterLlmBridge.nativeDefaultClientListFiles(
                    handle,
                    query?.let { MAPPER.writeValueAsString(it) } ?: "",
                )
    ...
    suspend fun fetchBatchForPolling(batchId: String): BatchObject {
        return withContext(Dispatchers.IO) {
            val responseJson =
                LiterLlmBridge.nativeDefaultClientFetchBatchForPolling(
                    handle,
                    MAPPER.writeValueAsString(batchId),
                )
            MAPPER.readValue(responseJson, BatchObject::class.java)
    ...
    suspend fun waitForBatch(batchId: String, config: WaitForBatchConfig): BatchObject {
        return withContext(Dispatchers.IO) {
            val responseJson =
                LiterLlmBridge.nativeDefaultClientWaitForBatch(
                    handle,
                    MAPPER.writeValueAsString(mapOf("batchId" to batchId, "config" to config)),
                )
            MAPPER.readValue(responseJson, BatchObject::class.java)
        }
    }
