...
internal extension ChatCompletionResponse {
    init(_ rb: RustBridge.ChatCompletionResponseRef) throws {
        ...
        self.choices = try rb.choices().map { (s: RustStringRef) -> Choice in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(Choice.self, from: d) }
