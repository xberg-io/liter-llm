...
public final class DefaultClient {
    ...
    public func retrieve(_ batchId: String) async throws -> BatchObject {
        return try await RustBridge.defaultClientRetrieve(self.inner, batchId)
    }
    ...
    public func waitForBatch(_ batchId: String, _ config: WaitForBatchConfig) async throws -> BatchObject {
        return try await RustBridge.defaultClientWaitForBatch(self.inner, batchId, config)
    }
...
public func recordCacheHit(system: String, model: String, operation: String) {
        let _rb_system = RustString(system)
        let _rb_model = RustString(model)
        let _rb_operation = RustString(operation)
    return RustBridge.recordCacheHit(_rb_system, _rb_model, _rb_operation)
}
/// Record a cache miss metric.
///
/// Call from cache layer implementations to emit `gen_ai.cache.miss`.
/// If the meter has not been initialized, this call is a no-op.
public func recordCacheMiss(system: String, model: String, operation: String) {
        let _rb_system = RustString(system)
        let _rb_model = RustString(model)
        let _rb_operation = RustString(operation)
    return RustBridge.recordCacheMiss(_rb_system, _rb_model, _rb_operation)
}
/// Record a stale cache metric.
///
/// Call from cache layer implementations to emit `gen_ai.cache.stale`.
/// If the meter has not been initialized, this call is a no-op.
public func recordCacheStale(system: String, model: String, operation: String) {
        let _rb_system = RustString(system)
        let _rb_model = RustString(model)
        let _rb_operation = RustString(operation)
    return RustBridge.recordCacheStale(_rb_system, _rb_model, _rb_operation)
}
/// Record a circuit breaker trip.
///
/// Call from `CircuitLayer` when the circuit opens.
/// If the meter has not been initialized, this call is a no-op.
public func recordCircuitTrip(system: String, model: String) {
        let _rb_system = RustString(system)
        let _rb_model = RustString(model)
    return RustBridge.recordCircuitTrip(_rb_system, _rb_model)
}
/// Record a retry attempt.
///
/// Call from retry/hedge layers to emit `gen_ai.retry.attempt`.
/// If the meter has not been initialized, this call is a no-op.
public func recordRetryAttempt(system: String, model: String, operation: String) {
        let _rb_system = RustString(system)
        let _rb_model = RustString(model)
        let _rb_operation = RustString(operation)
    return RustBridge.recordRetryAttempt(_rb_system, _rb_model, _rb_operation)
}
...
/// If the meter has not been initialized, this call is a no-op.
public func recordCacheTierHit(system: String, model: String, tier: String) {
        let _rb_system = RustString(system)
        let _rb_model = RustString(model)
        let _rb_tier = RustString(tier)
    return RustBridge.recordCacheTierHit(_rb_system, _rb_model, _rb_tier)
}
...
/// If the meter has not been initialized, this call is a no-op.
public func recordCacheTierMiss(system: String, model: String, tier: String) {
        let _rb_system = RustString(system)
        let _rb_model = RustString(model)
        let _rb_tier = RustString(tier)
    return RustBridge.recordCacheTierMiss(_rb_system, _rb_model, _rb_tier)
}
...
/// call is a no-op.
public func recordBudgetSpend(model: String, provider: String, tenantId: String? = nil, userId: String? = nil, apiKeyId: String? = nil, costUsd: Double) {
        let _rb_model = RustString(model)
        let _rb_provider = RustString(provider)
        let _rb_tenantId = tenantId.map { RustString($0) }
        let _rb_userId = userId.map { RustString($0) }
        let _rb_apiKeyId = apiKeyId.map { RustString($0) }
    return RustBridge.recordBudgetSpend(_rb_model, _rb_provider, _rb_tenantId, _rb_userId, _rb_apiKeyId, costUsd)
}
...
/// returning `Reject`.
/// If the meter has not been initialized, this call is a no-op.
public func recordBudgetRejection(model: String, provider: String, dimension: String) {
        let _rb_model = RustString(model)
        let _rb_provider = RustString(provider)
        let _rb_dimension = RustString(dimension)
    return RustBridge.recordBudgetRejection(_rb_model, _rb_provider, _rb_dimension)
}
/// Record the lifetime of a completed Realtime WebSocket session.
///
/// Emits `gen_ai.realtime.session.duration` (seconds).
/// If the meter has not been initialized, this call is a no-op.
public func recordRealtimeSessionDuration(provider: String, durationSecs: Double) {
        let _rb_provider = RustString(provider)
    return RustBridge.recordRealtimeSessionDuration(_rb_provider, durationSecs)
}
...
/// `gen_ai.system`.
/// If the meter has not been initialized, this call is a no-op.
public func recordRealtimeEvent(provider: String, direction: String, eventType: String) {
        let _rb_provider = RustString(provider)
        let _rb_direction = RustString(direction)
        let _rb_eventType = RustString(eventType)
    return RustBridge.recordRealtimeEvent(_rb_provider, _rb_direction, _rb_eventType)
}
...
/// If the meter has not been initialized, this call is a no-op.
public func recordRealtimeBytes(provider: String, direction: String, byteCount: UInt64) {
        let _rb_provider = RustString(provider)
        let _rb_direction = RustString(direction)
    return RustBridge.recordRealtimeBytes(_rb_provider, _rb_direction, byteCount)
}
