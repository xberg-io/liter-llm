import Foundation
import LiterLlm
import RustBridge
// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:14391a9aa8abd372376dca4cc565f342d18dfc125912d34032084e51d9ce2c94
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
// Issues & docs: https://github.com/kreuzberg-dev/alef
import XCTest

/// E2e tests for category: chat.
final class ChatTests: XCTestCase {
  func testDeveloperMessage() async throws {
    // Chat request that includes a developer role message alongside user messages
    let _request = try chatCompletionRequestFromJson(
      "{\"messages\":[{\"content\":\"You are a coding assistant. Always respond with concise code examples.\",\"role\":\"developer\"},{\"content\":\"How do I reverse a string in Python?\",\"role\":\"user\"}],\"model\":\"gpt-4\"}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]!
        + "/fixtures/developer_message")
    let result = try await _client.chat(_request)
    XCTAssertEqual(result.choices().count, 1)
    let _vec_choices_equals_7ba7bc0e = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_7ba7bc0e[0].message().content()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "s[::-1]")
    let _vec_choices_equals_f15f117a = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_f15f117a[0].finish_reason()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "stop")
  }

  func testEdgeChatMaxTokens() async throws {
    // Chat request with max_tokens=1 terminates with length finish_reason
    let _request = try chatCompletionRequestFromJson(
      "{\"max_tokens\":1,\"messages\":[{\"content\":\"Write a story\",\"role\":\"user\"}],\"model\":\"gpt-4\"}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]!
        + "/fixtures/edge_chat_max_tokens")
    let result = try await _client.chat(_request)
    let _vec_choices_equals_3ce619d2 = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_3ce619d2[0].finish_reason()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "length")
    let _vec_choices_not_empty_fc8f36c = result.choices()
    XCTAssertNotNil(_vec_choices_not_empty_fc8f36c[0].message().content(), "expected non-nil value")
  }

  func testEdgeChatSystemOnly() async throws {
    // Chat request with system message and user message
    let _request = try chatCompletionRequestFromJson(
      "{\"messages\":[{\"content\":\"You are a helpful and concise assistant\",\"role\":\"system\"},{\"content\":\"Hi\",\"role\":\"user\"}],\"model\":\"gpt-4\"}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]!
        + "/fixtures/edge_chat_system_only")
    let result = try await _client.chat(_request)
    let _vec_choices_not_empty_fc8f36c = result.choices()
    XCTAssertNotNil(_vec_choices_not_empty_fc8f36c[0].message().content(), "expected non-nil value")
  }

  func testEdgeChatTemperatureZero() async throws {
    // Chat request with temperature=0 for deterministic responses
    let _request = try chatCompletionRequestFromJson(
      "{\"messages\":[{\"content\":\"Say hello\",\"role\":\"user\"}],\"model\":\"gpt-4\",\"temperature\":0}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]!
        + "/fixtures/edge_chat_temperature_zero")
    let result = try await _client.chat(_request)
    let _vec_choices_not_empty_fc8f36c = result.choices()
    XCTAssertNotNil(_vec_choices_not_empty_fc8f36c[0].message().content(), "expected non-nil value")
  }

  func testFinishReasonContentFilter() async throws {
    // Chat response stopped by content filter with finish_reason of content_filter and null content
    let _request = try chatCompletionRequestFromJson(
      "{\"messages\":[{\"content\":\"Tell me something controversial\",\"role\":\"user\"}],\"model\":\"gpt-4\"}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]!
        + "/fixtures/finish_reason_content_filter")
    let result = try await _client.chat(_request)
    XCTAssertEqual(result.choices().count, 1)
    let _vec_choices_equals_eda78af = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_eda78af[0].finish_reason()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "content_filter")
  }

  func testFinishReasonLength() async throws {
    // Chat response truncated due to max_tokens limit with finish_reason of length
    let _request = try chatCompletionRequestFromJson(
      "{\"max_tokens\":5,\"messages\":[{\"content\":\"Tell me a long story\",\"role\":\"user\"}],\"model\":\"gpt-4\"}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]!
        + "/fixtures/finish_reason_length")
    let result = try await _client.chat(_request)
    XCTAssertEqual(result.choices().count, 1)
    let _vec_choices_equals_3ce619d2 = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_3ce619d2[0].finish_reason()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "length")
  }

  func testMultiTurnConversation() async throws {
    // Multi-turn conversation with system, user, assistant, and follow-up user messages
    let _request = try chatCompletionRequestFromJson(
      "{\"messages\":[{\"content\":\"You are a helpful assistant.\",\"role\":\"system\"},{\"content\":\"What is 2 + 2?\",\"role\":\"user\"},{\"content\":\"2 + 2 equals 4.\",\"role\":\"assistant\"},{\"content\":\"And what is 4 + 4?\",\"role\":\"user\"}],\"model\":\"gpt-4\"}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]!
        + "/fixtures/multi_turn_conversation")
    let result = try await _client.chat(_request)
    XCTAssertEqual(result.choices().count, 1)
    let _vec_choices_equals_c0c61677 = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_c0c61677[0].message().content()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "4 + 4 equals 8.")
    let _vec_choices_equals_f15f117a = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_f15f117a[0].finish_reason()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "stop")
  }

  func testParallelToolCalls() async throws {
    // Chat request that results in parallel tool calls in the response
    let _request = try chatCompletionRequestFromJson(
      "{\"messages\":[{\"content\":\"What is the weather in NYC and London?\",\"role\":\"user\"}],\"model\":\"gpt-4\",\"parallel_tool_calls\":true,\"tools\":[{\"function\":{\"description\":\"Get the current weather for a given location\",\"name\":\"get_weather\",\"parameters\":{\"properties\":{\"location\":{\"description\":\"The city name\",\"type\":\"string\"}},\"required\":[\"location\"],\"type\":\"object\"}},\"type\":\"function\"}]}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]!
        + "/fixtures/parallel_tool_calls")
    let result = try await _client.chat(_request)
    XCTAssertEqual(result.choices().count, 1)
    let _vec_choices_not_empty_c4dcde19 = result.choices()
    XCTAssertNotNil(
      _vec_choices_not_empty_c4dcde19[0].message().tool_calls(), "expected non-nil value")
    let _vec_choices_count_equals_325c94b = result.choices()
    XCTAssertEqual((result.choices()[0].message().tool_calls()?.count ?? 0), 2)
    let _vec_choices_equals_1c1cc717 = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_1c1cc717[0].finish_reason()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "tool_calls")
  }

  func testResponseFormatJsonObject() async throws {
    // Chat request with response_format json_object that returns valid JSON content
    let _request = try chatCompletionRequestFromJson(
      "{\"messages\":[{\"content\":\"Respond with JSON only.\",\"role\":\"system\"},{\"content\":\"Give me a user object with name and age fields.\",\"role\":\"user\"}],\"model\":\"gpt-4\",\"response_format\":{\"type\":\"json_object\"}}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]!
        + "/fixtures/response_format_json_object")
    let result = try await _client.chat(_request)
    XCTAssertEqual(result.choices().count, 1)
    let _vec_choices_not_empty_fc8f36c = result.choices()
    XCTAssertNotNil(_vec_choices_not_empty_fc8f36c[0].message().content(), "expected non-nil value")
    let _vec_choices_equals_f15f117a = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_f15f117a[0].finish_reason()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "stop")
  }

  func testResponseFormatJsonSchema() async throws {
    // Chat request with response_format json_schema that validates the output structure
    let _request = try chatCompletionRequestFromJson(
      "{\"messages\":[{\"content\":\"What is the temperature in Paris today?\",\"role\":\"user\"}],\"model\":\"gpt-4\",\"response_format\":{\"json_schema\":{\"name\":\"weather\",\"schema\":{\"properties\":{\"temp\":{\"type\":\"number\"}},\"required\":[\"temp\"],\"type\":\"object\"}},\"type\":\"json_schema\"}}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]!
        + "/fixtures/response_format_json_schema")
    let result = try await _client.chat(_request)
    XCTAssertEqual(result.choices().count, 1)
    let _vec_choices_not_empty_fc8f36c = result.choices()
    XCTAssertNotNil(_vec_choices_not_empty_fc8f36c[0].message().content(), "expected non-nil value")
    let _vec_choices_equals_f15f117a = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_f15f117a[0].finish_reason()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "stop")
  }

  func testSeedParameter() async throws {
    // Chat request with seed parameter for deterministic output; response includes system_fingerprint
    let _request = try chatCompletionRequestFromJson(
      "{\"messages\":[{\"content\":\"Pick a random number\",\"role\":\"user\"}],\"model\":\"gpt-4\",\"seed\":42}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]! + "/fixtures/seed_parameter")
    let result = try await _client.chat(_request)
    XCTAssertEqual(result.choices().count, 1)
    let _vec_choices_equals_f15f117a = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_f15f117a[0].finish_reason()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "stop")
    XCTAssertNotNil(result.system_fingerprint(), "expected non-nil value")
  }

  func testStopSequences() async throws {
    // Chat request with custom stop sequences that terminates generation at a stop token
    let _request = try chatCompletionRequestFromJson(
      "{\"messages\":[{\"content\":\"List items until you see STOP\",\"role\":\"user\"}],\"model\":\"gpt-4\",\"stop\":[\"STOP\",\"END\"]}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]! + "/fixtures/stop_sequences")
    let result = try await _client.chat(_request)
    XCTAssertEqual(result.choices().count, 1)
    let _vec_choices_equals_f15f117a = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_f15f117a[0].finish_reason()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "stop")
  }

  func testToolChoiceRequired() async throws {
    // Chat request with tool_choice set to required forces the model to call a tool
    let _request = try chatCompletionRequestFromJson(
      "{\"messages\":[{\"content\":\"What is the weather today?\",\"role\":\"user\"}],\"model\":\"gpt-4\",\"tool_choice\":\"required\",\"tools\":[{\"function\":{\"description\":\"Get the current weather for a given location\",\"name\":\"get_weather\",\"parameters\":{\"properties\":{\"location\":{\"description\":\"The city name\",\"type\":\"string\"}},\"required\":[\"location\"],\"type\":\"object\"}},\"type\":\"function\"}]}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]!
        + "/fixtures/tool_choice_required")
    let result = try await _client.chat(_request)
    XCTAssertEqual(result.choices().count, 1)
    let _vec_choices_not_empty_c4dcde19 = result.choices()
    XCTAssertNotNil(
      _vec_choices_not_empty_c4dcde19[0].message().tool_calls(), "expected non-nil value")
    let _vec_choices_equals_12e4bfba = result.choices()
    XCTAssertEqual(
      _vec_choices_equals_12e4bfba[0].message().tool_calls()?[0].function().name().toString()
        .trimmingCharacters(in: CharacterSet.whitespaces), "get_weather")
    let _vec_choices_equals_1c1cc717 = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_1c1cc717[0].finish_reason()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "tool_calls")
  }

  func testToolChoiceSpecific() async throws {
    // Chat request with tool_choice specifying a particular function to call
    let _request = try chatCompletionRequestFromJson(
      "{\"messages\":[{\"content\":\"What is the weather in Paris?\",\"role\":\"user\"}],\"model\":\"gpt-4\",\"tool_choice\":{\"function\":{\"name\":\"get_weather\"},\"type\":\"function\"},\"tools\":[{\"function\":{\"description\":\"Get the current weather for a given location\",\"name\":\"get_weather\",\"parameters\":{\"properties\":{\"location\":{\"description\":\"The city name\",\"type\":\"string\"}},\"required\":[\"location\"],\"type\":\"object\"}},\"type\":\"function\"},{\"function\":{\"description\":\"Search the web for information\",\"name\":\"search_web\",\"parameters\":{\"properties\":{\"query\":{\"description\":\"The search query\",\"type\":\"string\"}},\"required\":[\"query\"],\"type\":\"object\"}},\"type\":\"function\"}]}"
    )
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]!
        + "/fixtures/tool_choice_specific")
    let result = try await _client.chat(_request)
    XCTAssertEqual(result.choices().count, 1)
    let _vec_choices_not_empty_c4dcde19 = result.choices()
    XCTAssertNotNil(
      _vec_choices_not_empty_c4dcde19[0].message().tool_calls(), "expected non-nil value")
    let _vec_choices_equals_12e4bfba = result.choices()
    XCTAssertEqual(
      _vec_choices_equals_12e4bfba[0].message().tool_calls()?[0].function().name().toString()
        .trimmingCharacters(in: CharacterSet.whitespaces), "get_weather")
    let _vec_choices_equals_1c1cc717 = result.choices()
    XCTAssertEqual(
      (_vec_choices_equals_1c1cc717[0].finish_reason()?.toString() ?? "").trimmingCharacters(
        in: CharacterSet.whitespaces), "tool_calls")
  }

}
