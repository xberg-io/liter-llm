...
final class ErrorHandlingTests: XCTestCase {
  ...
  func testBadRequest400() async throws {
    ...
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]! + "/fixtures/bad_request_400"
    )
    do {
      _ = try await _client.chat(_request)
      XCTFail("expected to throw")
  ...
  func testRateLimit429() async throws {
    ...
    let _client = try DefaultClient(
      apiKey: "test-key",
      baseUrl: ProcessInfo.processInfo.environment["MOCK_SERVER_URL"]! + "/fixtures/rate_limit_429")
    do {
      _ = try await _client.chat(_request)
      XCTFail("expected to throw")
    } catch {
