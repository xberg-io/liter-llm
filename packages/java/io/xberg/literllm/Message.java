...
class MessageSerializer extends StdSerializer<Message> {
  ...
  public void serialize(Message value, JsonGenerator gen, SerializerProvider provider)
    ...
    String tag;
    Object inner;
    if (value instanceof Message.System variantSystem) {
      tag = "system";
      inner = variantSystem.value();
    } else if (value instanceof Message.User variantUser) {
      tag = "user";
      inner = variantUser.value();
    } else if (value instanceof Message.Assistant variantAssistant) {
      tag = "assistant";
      inner = variantAssistant.value();
    } else if (value instanceof Message.Tool variantTool) {
      tag = "tool";
      inner = variantTool.value();
    } else if (value instanceof Message.Developer variantDeveloper) {
      tag = "developer";
      inner = variantDeveloper.value();
    } else if (value instanceof Message.Function variantFunction) {
      tag = "function";
      inner = variantFunction.value();
    } else {
      throw new com.fasterxml.jackson.databind.JsonMappingException(
