...
//
// Errors are returned when returns an error only if the internal lock is poisoned.
...
// RecordCacheHit record a cache hit metric.
//
...
func RecordCacheHit(system string, model string, operation string) {
	...
	C.literllm_record_cache_hit(cSystem, cModel, cOperation)
}

// RecordCacheMiss record a cache miss metric.
//
...
func RecordCacheMiss(system string, model string, operation string) {
	...
	C.literllm_record_cache_miss(cSystem, cModel, cOperation)
}

// RecordCacheStale record a stale cache metric.
//
...
func RecordCacheStale(system string, model string, operation string) {
	...
	C.literllm_record_cache_stale(cSystem, cModel, cOperation)
}

// RecordCircuitTrip record a circuit breaker trip.
//
...
func RecordCircuitTrip(system string, model string) {
	...
	C.literllm_record_circuit_trip(cSystem, cModel)
}

// RecordRetryAttempt record a retry attempt.
//
...
func RecordRetryAttempt(system string, model string, operation string) {
	...
	C.literllm_record_retry_attempt(cSystem, cModel, cOperation)
}

// RecordCacheTierHit record a per-tier cache hit.
//
...
func RecordCacheTierHit(system string, model string, tier string) {
	...
	C.literllm_record_cache_tier_hit(cSystem, cModel, cTier)
}

// RecordCacheTierMiss record a per-tier cache miss.
//
// `tier` should be one of `"exact"`, `"semantic"`, or `"streaming_replay"`.
...
func RecordCacheTierMiss(system string, model string, tier string) {
	...
	C.literllm_record_cache_tier_miss(cSystem, cModel, cTier)
}

// RecordBudgetSpend record cumulative spend for a specific budget dimension.
//
...
func RecordBudgetSpend(model string, provider string, tenantID *string, userID *string, apiKeyID *string, costUsd float64) {
	...
	var capiKeyID *C.char
	if apiKeyID != nil {
		capiKeyID = C.CString(*apiKeyID)
		defer C.free(unsafe.Pointer(capiKeyID))
	}

	cCostUsd := C.double(float64(costUsd))

	C.literllm_record_budget_spend(cModel, cProvider, cTenantID, cUserID, capiKeyID, cCostUsd)
}

// RecordBudgetRejection record a budget-rejection event.
//
...
func RecordBudgetRejection(model string, provider string, dimension string) {
	...
	cDimension := C.CString(dimension)
	defer C.free(unsafe.Pointer(cDimension))

	C.literllm_record_budget_rejection(cModel, cProvider, cDimension)
}

// RecordRealtimeSessionDuration record the lifetime of a completed Realtime WebSocket session.
//
...
func RecordRealtimeSessionDuration(provider string, durationSecs float64) {
	...
	cDurationSecs := C.double(float64(durationSecs))

	C.literllm_record_realtime_session_duration(cProvider, cDurationSecs)
}

// RecordRealtimeEvent record a single Realtime event being forwarded.
//
...
func RecordRealtimeEvent(provider string, direction string, eventType string) {
	...
	C.literllm_record_realtime_event(cProvider, cDirection, cEventType)
}

// RecordRealtimeBytes record audio bytes forwarded over a Realtime WebSocket session.
//
...
func RecordRealtimeBytes(provider string, direction string, byteCount uint64) {
	...
	cByteCount := C.uint64_t(uint64(byteCount))

	C.literllm_record_realtime_bytes(cProvider, cDirection, cByteCount)
}
...
func (h *DefaultClient) Retrieve(batchID string) (*BatchObject, error) {
	cBatchID := C.CString(batchID)
	defer C.free(unsafe.Pointer(cBatchID))

	ptr := C.literllm_default_client_retrieve((*C.LITERLLMDefaultClient)(unsafe.Pointer(h.ptr)), cBatchID)
	if err := lastError(); err != nil {
		return nil, err
	}
	defer C.literllm_batch_object_free(ptr)
