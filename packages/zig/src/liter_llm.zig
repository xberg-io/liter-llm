...
inline fn _error_with_message(comptime E: type) E {
    ...
    if (E == CatalogRefreshError) return _from_ffi_msg_CatalogRefreshError(msg_opt);
...
/// Map an FFI error message string to a `CatalogRefreshError` variant by prefix-matching
/// against the upstream `#[error("...")]` templates. Falls back to the
/// first declared variant when no prefix matches.
inline fn _from_ffi_msg_CatalogRefreshError(msg_opt: ?[]const u8) CatalogRefreshError {
    if (msg_opt) |msg| {
        if (std.mem.startsWith(u8, msg, "catalog refresh is disabled")) return error.Disabled;
        if (std.mem.startsWith(u8, msg, "insecure catalog source URL")) return error.InsecureUrl;
        if (std.mem.startsWith(u8, msg, "failed to fetch catalog from")) return error.Fetch;
        if (std.mem.startsWith(u8, msg, "failed to parse catalog JSON:")) return error.Parse;
        if (std.mem.startsWith(u8, msg, "catalog cache I/O error at")) return error.Cache;
    }
    return _first_error(CatalogRefreshError);
}
...
pub fn decode_data_url(url: []const u8) error{OutOfMemory}!?[]u8 {
    ...
    return if (_result == null) null else blk: {
        const _json_ptr = c.literllm_decoded_data_url_to_json(_result.?);
        c.literllm_decoded_data_url_free(_result.?);
...
pub fn model_info(model: []const u8) error{OutOfMemory}!?[]u8 {
    ...
    return if (_result == null) null else blk: {
        const _json_ptr = c.literllm_model_info_to_json(_result.?);
        c.literllm_model_info_free(_result.?);
        defer _free_string(_json_ptr);
        const slice = std.mem.sliceTo(_json_ptr, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
        break :blk owned;
...
pub const DefaultClient = struct {
    ...
    pub fn fetch_batch_for_polling(self: *DefaultClient, batch_id: []const u8) (LiterLlmError||error{OutOfMemory})![]u8 {
        const batch_id_z = try std.heap.c_allocator.dupeZ(u8, batch_id);
        defer std.heap.c_allocator.free(batch_id_z);
        const _result = c.literllm_default_client_fetch_batch_for_polling(@as(*c.LITERLLMDefaultClient, @ptrCast(self._handle)), batch_id_z);
        if (_result == null) {
            return _first_error(LiterLlmError);
        }
        return blk: {
