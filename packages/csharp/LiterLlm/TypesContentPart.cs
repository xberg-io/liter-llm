...
/// </summary>
    [JsonConverter(typeof(TypesContentPartJsonConverter))]
public abstract record TypesContentPart
{
    /// <summary>
    /// Plain text.
    /// </summary>
    ...
    /// </summary>
    public sealed record ImageUrl : TypesContentPart
    {
...
/// <summary>
/// Custom converter for TypesContentPart sealed union with flattened variant fields.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed class TypesContentPartJsonConverter : JsonConverter<TypesContentPart>
{
    public override TypesContentPart Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {            "text" => JsonSerializer.Deserialize<TypesContentPart.Text>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            "image_url" => JsonSerializer.Deserialize<TypesContentPart.ImageUrl>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            "document" => JsonSerializer.Deserialize<TypesContentPart.Document>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            "input_audio" => JsonSerializer.Deserialize<TypesContentPart.InputAudio>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            // Unknown discriminator: forward compatibility for unknown variants
            _ => throw new JsonException($"Unknown TypesContentPart discriminator: {tagValue}")
        };
    }

    public override void Write(Utf8JsonWriter writer, TypesContentPart value, JsonSerializerOptions options)
    {
        ...
        object? inner;
        switch (value)
        {            case TypesContentPart.Text v_text:
                tag = "text";                inner = v_text;                break;            case TypesContentPart.ImageUrl v_imageurl:
                tag = "image_url";                inner = v_imageurl;                break;            case TypesContentPart.Document v_document:
                tag = "document";                inner = v_document;                break;            case TypesContentPart.InputAudio v_inputaudio:
                tag = "input_audio";                inner = v_inputaudio;                break;            default:
                throw new JsonException($"Unknown TypesContentPart variant: {value.GetType().Name}");
        }
