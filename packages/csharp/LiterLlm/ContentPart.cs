...
<<<<<<< Updated upstream
        {
            ...
            "input_audio" => JsonSerializer.Deserialize<ContentPart.InputAudio>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            // Unknown discriminator: forward compatibility for unknown variants
            _ => throw new JsonException($"Unknown ContentPart discriminator: {tagValue}")
=======
        {            "text" => JsonSerializer.Deserialize<ContentPart.Text>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            "image_url" => JsonSerializer.Deserialize<ContentPart.ImageUrl>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            "document" => JsonSerializer.Deserialize<ContentPart.Document>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            "input_audio" => JsonSerializer.Deserialize<ContentPart.InputAudio>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            _ => throw new JsonException($"Unknown ContentPart discriminator: {tagValue}")
>>>>>>> Stashed changes
        };
    }
    ...
    public override void Write(Utf8JsonWriter writer, ContentPart value, JsonSerializerOptions options)
    {
        ...
        {            case ContentPart.Text v_text:
                tag = "text";                inner = v_text;                break;            case ContentPart.ImageUrl v_imageurl:
                tag = "image_url";                inner = v_imageurl;                break;            case ContentPart.Document v_document:
                tag = "document";                inner = v_document;                break;            case ContentPart.InputAudio v_inputaudio:
