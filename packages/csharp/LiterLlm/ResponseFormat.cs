...
public sealed class ResponseFormatJsonConverter : JsonConverter<ResponseFormat>
{
    public override ResponseFormat Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {            "text" => new ResponseFormat.Text(),            "json_object" => new ResponseFormat.JsonObject(),            "json_schema" => JsonSerializer.Deserialize<ResponseFormat.JsonSchema>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            // Unknown discriminator: binding-excluded variant or forward compatibility
            _ => throw new JsonException($"Unknown ResponseFormat discriminator: {tagValue} (this may be a binding-excluded variant)")
        };
    }
