...
public sealed class ResponseFormatJsonConverter : JsonConverter<ResponseFormat>
{
    public override ResponseFormat Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {            "text" => new ResponseFormat.Text(),            "json_object" => new ResponseFormat.JsonObject(),            "json_schema" => JsonSerializer.Deserialize<ResponseFormat.JsonSchema>(flatJson, options) ?? throw new JsonException("Failed to deserialize variant"),            // Unknown discriminator: forward compatibility for unknown variants
