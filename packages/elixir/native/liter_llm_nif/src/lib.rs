...
// alef:hash:f2a54730f9d5b619891da4a619d02d66e1b0386048722d9109b2ebe2d635cef7
...
impl std::panic::RefUnwindSafe for TowerCachedResponse {}

impl rustler::Resource for TowerCachedResponse {}
...
impl std::panic::RefUnwindSafe for TowerLlmRequest {}

impl rustler::Resource for TowerLlmRequest {}
...
impl std::panic::RefUnwindSafe for TowerLlmResponse {}

impl rustler::Resource for TowerLlmResponse {}
...
pub fn count_tokens(model: String, text: String) -> Result<usize, String> {
    let result = liter_llm::tokenizer::count_tokens(&model, &text).map_err(|e| e.to_string())?;
    Ok(result)
}
...
pub fn count_request_tokens(model: String, req: Option<String>) -> Result<usize, String> {
    let req_core: Option<liter_llm::types::ChatCompletionRequest> = req
        ...
        .transpose()
        .map_err(|e| e.to_string())?;
    let result = liter_llm::tokenizer::count_request_tokens(&model, req_core.as_ref().unwrap_or(&Default::default()))
        .map_err(|e| e.to_string())?;
    Ok(result)
}
...
pub fn defaultclient_retrieve_response_async(
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            let rt = tokio::runtime::Runtime::new().map_err(|e| e.to_string())?;
            let result = rt
                .block_on(async { resource.inner.as_ref().retrieve_response(&id).await })
                .map_err(|e| e.to_string())?;
            Ok(result.into())
        })
        .map_err(|e| e.to_string())?
...
pub fn defaultclient_cancel_response_async(
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            let rt = tokio::runtime::Runtime::new().map_err(|e| e.to_string())?;
            let result = rt
                .block_on(async { resource.inner.as_ref().cancel_response(&id).await })
                .map_err(|e| e.to_string())?;
            Ok(result.into())
        })
        .map_err(|e| e.to_string())?
...
#[rustler::nif]
pub fn towercachedresponse_into_llm_response(resource: rustler::ResourceArc<TowerCachedResponse>) -> String {
    String::from("[unimplemented: towercachedresponse_into_llm_response]")
}
...
#[rustler::nif]
pub fn towerllmrequest_operation_name(resource: rustler::ResourceArc<TowerLlmRequest>) -> String {
    resource.inner.as_ref().operation_name().into()
}

/// Human-readable name of the request type; used as a span / metric label.
#[rustler::nif]
pub fn towerllmrequest_request_type(resource: rustler::ResourceArc<TowerLlmRequest>) -> String {
    resource.inner.as_ref().request_type().into()
}

/// Return the model name embedded in the request, if any.
#[rustler::nif]
pub fn towerllmrequest_model(resource: rustler::ResourceArc<TowerLlmRequest>) -> Option<String> {
    resource.inner.as_ref().model().map(|v| v.into())
}
...
#[rustler::nif]
pub fn towerllmresponse_usage(resource: rustler::ResourceArc<TowerLlmResponse>) -> Option<Usage> {
    resource.inner.as_ref().usage().map(|v| v.clone().into())
}
...
fn on_load(env: rustler::Env, _info: rustler::Term) -> bool {
    ...
    env.register::<TowerCachedResponse>()
        .expect("Failed to register resource type TowerCachedResponse");
    env.register::<TowerLlmRequest>()
        .expect("Failed to register resource type TowerLlmRequest");
    env.register::<TowerLlmResponse>()
        .expect("Failed to register resource type TowerLlmResponse");
