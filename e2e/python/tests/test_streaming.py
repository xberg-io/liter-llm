...
# alef:hash:874f68da333ec63d2c4a4ee830867ede4744296a6c343149b4e46f274464ef8f
...
async def test_anthropic_stream() -> None:
    ...
    assert (
        "".join(c.choices[0].delta.content or "" for c in chunks if c.choices).strip()
        == "One Two Three".strip()
    )  # noqa: S101
...
async def test_stream_with_tool_calls() -> None:
    ...
    assert (
        str(chunks[-1].choices[0].finish_reason)
        if chunks and chunks[-1].choices
        else None
    ...
    )[0].function.name.strip() == "get_weather".strip()  # noqa: S101


@pytest.mark.skip(
    reason="Rust returns BoxStream — collecting usage from the final chunk requires consuming the stream, which is not supported by the generator. Python deferred until streaming-virtual usage path can disambiguate from non-streaming response.usage."
)
@pytest.mark.asyncio
