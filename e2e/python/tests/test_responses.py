# This file is auto-generated by alef — DO NOT EDIT.
# alef:hash:614a9a228c315e2e022431f3529f2dc8acdbb8c9b89123fe631385dfe0d5ad5f
# To regenerate: alef generate
# To verify freshness: alef verify --exit-code

"""E2e tests for category: responses."""

import os
import pytest  # noqa: F401
from liter_llm import create_client, CreateResponseRequest
from liter_llm._internal_bindings import CreateResponseRequest


def _alef_e2e_text(value: object) -> str:
    return "" if value is None else str(value)


def _alef_e2e_item_texts(item: object) -> tuple[str, ...]:
    raw_items = getattr(item, "items", None)
    items_text = " ".join(str(value) for value in raw_items) if isinstance(raw_items, list) else ""
    return (
        _alef_e2e_text(item),
        _alef_e2e_text(getattr(item, "kind", None)),
        _alef_e2e_text(getattr(item, "name", None)),
        _alef_e2e_text(getattr(item, "source", None)),
        _alef_e2e_text(getattr(item, "alias", None)),
        _alef_e2e_text(getattr(item, "text", None)),
        _alef_e2e_text(getattr(item, "signature", None)),
        items_text,
    )


@pytest.mark.asyncio
async def test_edge_response_empty_output() -> None:
    """Response completes with empty output items."""
    client = create_client(
        api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/edge_response_empty_output"
    )
    request = CreateResponseRequest.from_json('{"input":"","model":"gpt-4o"}')

    result = await client.create_response(request)
    assert result.id  # noqa: S101
    assert result.status.strip() == "completed"  # noqa: S101
    assert len(result.output) == 0  # noqa: S101


@pytest.mark.asyncio
async def test_edge_response_large_input() -> None:
    """Response created with a very large input text."""
    client = create_client(
        api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/edge_response_large_input"
    )
    request = CreateResponseRequest.from_json(
        '{"input":"Summarize the following long text: Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet. ","model":"gpt-4o"}'
    )

    result = await client.create_response(request)
    assert result.id  # noqa: S101
    assert result.status.strip() == "completed"  # noqa: S101
    assert len(result.output) == 1  # noqa: S101


@pytest.mark.asyncio
async def test_error_response_auth_401() -> None:
    """401 Unauthorized when creating a response with invalid API key."""
    with pytest.raises(Exception) as exc_info:  # noqa: B017
        client = create_client(
            api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/error_response_auth_401"
        )
        request = CreateResponseRequest.from_json('{"input":"Hello","model":"gpt-4o"}')
        await client.create_response(request)
    assert "Authentication" in str(exc_info.value) or "Authentication" in type(exc_info.value).__name__  # noqa: S101


@pytest.mark.asyncio
async def test_error_response_bad_request() -> None:
    """400 Bad Request when creating response with invalid model."""
    with pytest.raises(Exception) as exc_info:  # noqa: B017
        client = create_client(
            api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/error_response_bad_request"
        )
        request = CreateResponseRequest.from_json('{"input":"Hello","model":"nonexistent-model"}')
        await client.create_response(request)
    assert "BadRequest" in str(exc_info.value) or "BadRequest" in type(exc_info.value).__name__  # noqa: S101


@pytest.mark.asyncio
async def test_error_response_not_found() -> None:
    """404 Not Found when retrieving a nonexistent response."""
    with pytest.raises(Exception) as exc_info:  # noqa: B017
        client = create_client(
            api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/error_response_not_found"
        )
        response_id = "resp-nonexistent"
        await client.retrieve_response(response_id)
    assert "NotFound" in str(exc_info.value) or "NotFound" in type(exc_info.value).__name__  # noqa: S101


@pytest.mark.asyncio
async def test_smoke_cancel_response() -> None:
    """Cancel an in-progress response."""
    client = create_client(
        api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/smoke_cancel_response"
    )
    response_id = "resp-def456"

    result = await client.cancel_response(response_id)
    assert result.id  # noqa: S101
    assert result.status.strip() == "cancelled"  # noqa: S101
    assert len(result.output) == 0  # noqa: S101


@pytest.mark.asyncio
async def test_smoke_create_response() -> None:
    """Create a basic response using the Responses API."""
    client = create_client(
        api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/smoke_create_response"
    )
    request = CreateResponseRequest.from_json('{"input":"Explain quantum computing in one sentence.","model":"gpt-4o"}')

    result = await client.create_response(request)
    assert result.id  # noqa: S101
    assert result.status.strip() == "completed"  # noqa: S101
    assert len(result.output) == 1  # noqa: S101


@pytest.mark.asyncio
async def test_smoke_response_with_tools() -> None:
    """Response that includes tool call output items."""
    client = create_client(
        api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/smoke_response_with_tools"
    )
    request = CreateResponseRequest.from_json(
        '{"input":"What is the weather in San Francisco?","model":"gpt-4o","tools":[{"description":"Get current weather for a location","name":"get_weather","parameters":{"properties":{"location":{"type":"string"}},"required":["location"],"type":"object"},"type":"function"}]}'
    )

    result = await client.create_response(request)
    assert result.id  # noqa: S101
    assert result.status.strip() == "completed"  # noqa: S101
    assert len(result.output) == 2  # noqa: S101
    assert result.output  # noqa: S101


@pytest.mark.asyncio
async def test_smoke_retrieve_response() -> None:
    """Retrieve a previously created response."""
    client = create_client(
        api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/smoke_retrieve_response"
    )
    response_id = "resp-abc123"

    result = await client.retrieve_response(response_id)
    assert result.id  # noqa: S101
    assert result.status.strip() == "completed"  # noqa: S101
