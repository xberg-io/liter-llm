# This file is auto-generated by alef — DO NOT EDIT.
# alef:hash:3fb07515ba02077671946cad7f111c0c7840343537bb31d1488bc316651dca95
# To regenerate: alef generate
# To verify freshness: alef verify --exit-code

"""E2e tests for category: list-models."""

import os
import pytest  # noqa: F401
from liter_llm import create_client


def _alef_e2e_text(value: object) -> str:
    return "" if value is None else str(value)


def _alef_e2e_item_texts(item: object) -> tuple[str, ...]:
    raw_items = getattr(item, "items", None)
    items_text = " ".join(str(value) for value in raw_items) if isinstance(raw_items, list) else ""
    return (
        _alef_e2e_text(item),
        _alef_e2e_text(getattr(item, "kind", None)),
        _alef_e2e_text(getattr(item, "name", None)),
        _alef_e2e_text(getattr(item, "source", None)),
        _alef_e2e_text(getattr(item, "alias", None)),
        _alef_e2e_text(getattr(item, "text", None)),
        _alef_e2e_text(getattr(item, "signature", None)),
        items_text,
    )


@pytest.mark.asyncio
async def test_empty_model_list() -> None:
    """List models response returns an empty data array when no models are available."""
    client = create_client(api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/empty_model_list")

    result = await client.list_models()
    assert len(result.data) >= 0  # noqa: S101
    assert len(result.data) == 0  # noqa: S101


@pytest.mark.asyncio
async def test_list_models_error_401() -> None:
    """401 Unauthorized error on list models request when API key is invalid."""
    with pytest.raises(Exception) as exc_info:  # noqa: B017
        client = create_client(
            api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/list_models_error_401"
        )
        await client.list_models()
    assert "Authentication" in str(exc_info.value) or "Authentication" in type(exc_info.value).__name__  # noqa: S101


@pytest.mark.asyncio
async def test_list_models_error_500() -> None:
    """500 Internal Server Error when listing models due to server failure."""
    with pytest.raises(Exception) as exc_info:  # noqa: B017
        client = create_client(
            api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/list_models_error_500"
        )
        await client.list_models()
    assert "ServerError" in str(exc_info.value) or "ServerError" in type(exc_info.value).__name__  # noqa: S101


@pytest.mark.asyncio
async def test_list_models_filtered() -> None:
    """List models response with multiple model objects."""
    client = create_client(
        api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/list_models_filtered"
    )

    result = await client.list_models()
    assert len(result.data) >= 5  # noqa: S101
    assert _alef_e2e_text(result.data[0].object).lower() == "model".lower()  # noqa: S101
