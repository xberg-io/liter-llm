...
// alef:hash:0b173d96c8dcaf7c9edd29d275b3845090518322578eeb7bfbca3b81780dd552
...
void test_proxy_chat_streaming(void) {
    ...
    LITERLLMChatCompletionRequest* chat_completion_request_handle = literllm_chat_completion_request_from_json("{\"messages\":[{\"content\":\"Count to 3\",\"role\":\"user\"}],\"model\":\"openai/gpt-4o\",\"stream\":true}");
    assert(chat_completion_request_handle != NULL && "failed to build request");
    LITERLLMDefaultClient* client = literllm_create_client("test-key", NULL, 0, 0, NULL);
    assert(client != NULL && "failed to create client");
...
void test_proxy_image_generate(void) {
    ...
    LITERLLMCreateImageRequest* create_image_request_handle = literllm_create_image_request_from_json("{\"model\":\"dall-e-3\",\"n\":1,\"prompt\":\"A sunset over the ocean\",\"size\":\"1024x1024\"}");
