...
void test_embed_base64(void) {
    ...
    LITERLLMEmbeddingRequest* embedding_request_handle = literllm_embedding_request_from_json("{\"encoding_format\":\"base64\",\"input\":\"Test input\",\"model\":\"text-embedding-3-small\"}");
    assert(embedding_request_handle != NULL && "failed to build request");
    const char* mock_base = getenv("MOCK_SERVER_URL");
    assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
    char base_url[1024];
