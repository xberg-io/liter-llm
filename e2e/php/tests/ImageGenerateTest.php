<?php

// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:614a9a228c315e2e022431f3529f2dc8acdbb8c9b89123fe631385dfe0d5ad5f
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code

declare(strict_types=1);

namespace Liter\Llm\E2e;

use Liter\Llm\CreateImageRequest;
use Liter\Llm\LiterLlm;
use PHPUnit\Framework\TestCase;

/** E2e tests for category: image-generate. */
final class ImageGenerateTest extends TestCase
{
    /** Image generation returning base64-encoded data instead of URL */
    public function test_edge_image_b64_response(): void
    {
        $client = \Liter\Llm\LiterLlm::createClient(
            'test-key',
            getenv('MOCK_SERVER_URL') . '/fixtures/edge_image_b64_response',
        );
        $request = \Liter\Llm\CreateImageRequest::from_json(json_encode([
            'model' => 'dall-e-3',
            'n' => 1,
            'prompt' => 'A blue circle',
            'responseFormat' => 'b64_json',
            'size' => '1024x1024',
        ]));
        $result = $client->imageGenerate($request);

        $this->assertCount(1, $result->getData());
        $this->assertNotEmpty($result->getData()[0]->b64Json ?? null);
    }

    /** Image generation with an empty prompt returns 400 */
    public function test_edge_image_empty_prompt(): void
    {
        $client = \Liter\Llm\LiterLlm::createClient(
            'test-key',
            getenv('MOCK_SERVER_URL') . '/fixtures/edge_image_empty_prompt',
        );
        $this->expectException(\Exception::class);
        $request = \Liter\Llm\CreateImageRequest::from_json(json_encode([
            'model' => 'dall-e-3',
            'n' => 1,
            'prompt' => '',
            'size' => '1024x1024',
        ]));
        $client->imageGenerate($request);
    }

    /** Image generation requesting multiple images with n=3 */
    public function test_edge_image_multiple_n(): void
    {
        $client = \Liter\Llm\LiterLlm::createClient(
            'test-key',
            getenv('MOCK_SERVER_URL') . '/fixtures/edge_image_multiple_n',
        );
        $request = \Liter\Llm\CreateImageRequest::from_json(json_encode([
            'model' => 'dall-e-3',
            'n' => 3,
            'prompt' => 'A cat',
        ]));
        $result = $client->imageGenerate($request);

        $this->assertCount(3, $result->getData());
        $this->assertNotEmpty($result->getData()[0]->url ?? null);
        $this->assertNotEmpty($result->getData()[1]->url ?? null);
        $this->assertNotEmpty($result->getData()[2]->url ?? null);
    }

    /** 401 Unauthorized when generating images with invalid API key */
    public function test_error_image_auth_401(): void
    {
        $client = \Liter\Llm\LiterLlm::createClient(
            'test-key',
            getenv('MOCK_SERVER_URL') . '/fixtures/error_image_auth_401',
        );
        $this->expectException(\Exception::class);
        $request = \Liter\Llm\CreateImageRequest::from_json(json_encode([
            'model' => 'dall-e-3',
            'n' => 1,
            'prompt' => 'A cat',
            'size' => '1024x1024',
        ]));
        $client->imageGenerate($request);
    }

    /** 400 Bad Request when image generation parameters are invalid */
    public function test_error_image_bad_request(): void
    {
        $client = \Liter\Llm\LiterLlm::createClient(
            'test-key',
            getenv('MOCK_SERVER_URL') . '/fixtures/error_image_bad_request',
        );
        $this->expectException(\Exception::class);
        $request = \Liter\Llm\CreateImageRequest::from_json(json_encode([
            'model' => 'dall-e-3',
            'n' => 1,
            'prompt' => 'A cat',
            'size' => '9999x9999',
        ]));
        $client->imageGenerate($request);
    }

    /** 429 Rate limit exceeded for image generation */
    public function test_error_image_rate_limit(): void
    {
        $client = \Liter\Llm\LiterLlm::createClient(
            'test-key',
            getenv('MOCK_SERVER_URL') . '/fixtures/error_image_rate_limit',
        );
        $this->expectException(\Exception::class);
        $request = \Liter\Llm\CreateImageRequest::from_json(json_encode([
            'model' => 'dall-e-3',
            'n' => 1,
            'prompt' => 'A cat',
            'size' => '1024x1024',
        ]));
        $client->imageGenerate($request);
    }

    /** Basic image generation with a text prompt */
    public function test_smoke_image_basic(): void
    {
        $client = \Liter\Llm\LiterLlm::createClient(
            'test-key',
            getenv('MOCK_SERVER_URL') . '/fixtures/smoke_image_basic',
        );
        $request = \Liter\Llm\CreateImageRequest::from_json(json_encode([
            'model' => 'dall-e-3',
            'n' => 1,
            'prompt' => 'A white cat sitting on a windowsill',
            'size' => '1024x1024',
        ]));
        $result = $client->imageGenerate($request);

        $this->assertCount(1, $result->getData());
        $this->assertNotEmpty($result->getData()[0]->url ?? null);
    }

    /** Image generation requesting multiple images */
    public function test_smoke_image_multiple(): void
    {
        $client = \Liter\Llm\LiterLlm::createClient(
            'test-key',
            getenv('MOCK_SERVER_URL') . '/fixtures/smoke_image_multiple',
        );
        $request = \Liter\Llm\CreateImageRequest::from_json(json_encode([
            'model' => 'dall-e-2',
            'n' => 3,
            'prompt' => 'A red bicycle',
            'size' => '256x256',
        ]));
        $result = $client->imageGenerate($request);

        $this->assertCount(3, $result->getData());
        $this->assertNotEmpty($result->getData()[0]->url ?? null);
    }

    /** Image generation with explicit size parameter */
    public function test_smoke_image_with_size(): void
    {
        $client = \Liter\Llm\LiterLlm::createClient(
            'test-key',
            getenv('MOCK_SERVER_URL') . '/fixtures/smoke_image_with_size',
        );
        $request = \Liter\Llm\CreateImageRequest::from_json(json_encode([
            'model' => 'dall-e-3',
            'n' => 1,
            'prompt' => 'A sunset over mountains',
            'size' => '1792x1024',
        ]));
        $result = $client->imageGenerate($request);

        $this->assertCount(1, $result->getData());
        $this->assertNotEmpty($result->getData()[0]->url ?? null);
    }
}
