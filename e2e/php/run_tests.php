...
// alef:hash:9158f8437705c08f9bceaac3ab1d3c7dceaa932a1d1fc9445ad3f3a68d960170
...
$repoRoot = __DIR__ . '/../../';
$localExtCandidates = [
    $repoRoot . 'target/release/libliter_llm_php' . $extSuffix,
    $repoRoot . 'crates/liter-llm-php/target/release/libliter_llm_php' . $extSuffix,
];
...
if (!file_exists($extPath)) {
    fwrite(STDERR, "error: no liter_llm PHP extension build found.\n");
    foreach ($localExtCandidates as $candidate) {
    ...
    fwrite(STDERR, "  cargo build --release -p liter-llm-php\n");
    exit(1);
}
...
$php = PHP_BINARY;
$isolatedIni = alef_build_isolated_ini($php, 'liter_llm');
...
$process = proc_open(
    ...
    $pipes,
    null,
    ['ALEF_PREFLIGHT_EXT_NAME' => 'liter_llm'] + $_ENV,
    ['bypass_shell' => true]
);
if ($process === false) {
    fwrite(STDERR, "error: failed to spawn PHP preflight check for the liter_llm extension.\n");
    exit(1);
}
...
if (stripos($preflightStderr, 'already loaded') !== false) {
    fwrite(STDERR, "error: the liter_llm extension is already loaded from an ambient php.ini\n");
...
if ($preflightExit !== 0 || $preflightStdout === '') {
    fwrite(STDERR, "error: the liter_llm extension did not load from $extPath.\n");
...
if ($loadedVersion !== '2.0.1') {
    fwrite(STDERR, "error: loaded liter_llm extension version mismatch.\n");
    fwrite(STDERR, "  extension at $extPath reports version: $loadedVersion\n");
    fwrite(STDERR, "  expected version: 2.0.1\n");
