...
describe("contract", () => {
  it("binding_api_parity: Verify all bindings expose the full API surface — constructor accepts all config options and every method exists", async () => {
    ...
    const result = await client.chat(
      (() => {
        ...
        _u0.messages = [{ content: "Contract test", role: "user" }];
        _u0.model = "openai/gpt-4o";
        return _u0;
      })(),
    );
  ...
  it("contract_ocr: Verify ocr() method exists in all bindings", async () => {
    ...
    await client.ocr(
      (() => {
        const _u0 = WasmOcrRequest.default();
        _u0.document = { type: "document_url", url: "https://example.com/contract-test.pdf" };
        _u0.model = "mistral/mistral-ocr-latest";
        return _u0;
      })(),
    );
  }, 30000);
  it("contract_search: Verify search() method exists in all bindings", async () => {
    ...
    await client.search(
      (() => {
        const _u0 = WasmSearchRequest.default();
        _u0.model = "brave/web-search";
        _u0.query = "contract test query";
        return _u0;
      })(),
    );
  }, 30000);
