// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:ce5a12d4d523d2ce62191111fb7f29b20c03404c3d05941e63fcaa24de76cc43
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
// Issues & docs: https://github.com/kreuzberg-dev/alef

import { describe, expect, it } from "vitest";
import {
  createClient,
  WasmChatCompletionRequest,
  WasmOcrRequest,
  WasmSearchRequest,
  WasmChatCompletionTool,
  WasmStreamOptions,
  WasmReasoningEffort,
  WasmOcrDocument,
  WasmMessage,
  WasmResponseFormat,
  WasmToolChoice,
  WasmStopSequence,
  WasmFinishReason,
} from "liter_llm";
function _alefE2eText(value: unknown): string {
  return value == null ? "" : String(value);
}

function _alefE2eItemTexts(item: unknown): string[] {
  if (item == null || typeof item !== "object") {
    return [_alefE2eText(item)];
  }
  const record = item as Record<string, unknown>;
  const itemsText = Array.isArray(record.items) ? record.items.map(_alefE2eText).join(" ") : "";
  return [
    _alefE2eText(item),
    _alefE2eText(record.kind),
    _alefE2eText(record.name),
    _alefE2eText(record.source),
    _alefE2eText(record.alias),
    _alefE2eText(record.text),
    _alefE2eText(record.signature),
    itemsText,
  ];
}

describe("contract", () => {
  it("binding_api_parity: Verify all bindings expose the full API surface — constructor accepts all config options and every method exists", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/binding_api_parity`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "Contract test";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "openai/gpt-4o";
        return _u;
      })(),
    );
    expect(result.choices.length).toBe(1);
    expect((result.choices[0].message.content ?? "").trim()).toBe("OK");
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.Stop);
    expect(Number(result.usage.totalTokens)).toBe(6);
    expect(result.model.trim()).toBe("gpt-4o");
  });
  it("contract_ocr: Verify ocr() method exists in all bindings", async () => {
    const client = createClient("test-key", `${process.env.MOCK_SERVER_URL}/fixtures/contract_ocr`);
    await client.ocr(
      (() => {
        const _u = new WasmOcrRequest();
        _u.document = (() => {
          const _u = new WasmOcrDocument();
          _u.type = "document_url";
          _u.url = "https://example.com/contract-test.pdf";
          return _u;
        })();
        _u.model = "mistral/mistral-ocr-latest";
        return _u;
      })(),
    );
  });
  it("contract_search: Verify search() method exists in all bindings", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/contract_search`,
    );
    await client.search(
      (() => {
        const _u = new WasmSearchRequest();
        _u.model = "brave/web-search";
        _u.query = "contract test query";
        return _u;
      })(),
    );
  });
});
