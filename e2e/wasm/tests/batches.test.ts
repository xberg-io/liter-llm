...
// alef:hash:1a492af4ccbc9c944931866461ffe5f9b5795a3b49ccb26f41ce5ee12ef4426a
...
describe("batches", () => {
  ...
  it("smoke_batch_completed: Retrieve a completed batch job with output file", async () => {
    ...
    expect(result.status).toBe(WasmBatchStatus.Completed);
  });
  it("smoke_cancel_batch: Cancel a running batch job", async () => {
    ...
    expect(result.status).toBe(WasmBatchStatus.Cancelling);
  });
  it("smoke_create_batch: Create a new batch processing job", async () => {
    ...
    expect(result.status).toBe(WasmBatchStatus.Validating);
  });
  ...
  it("smoke_retrieve_batch: Retrieve the status of a batch job", async () => {
    ...
    expect(result.status).toBe(WasmBatchStatus.InProgress);
  });
});
