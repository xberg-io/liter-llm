// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:f096489a99d59c725f2b870b5274f3bc7ecd98f4116a64e94a53c1ce6a83bb84
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
// Issues & docs: https://github.com/kreuzberg-dev/alef

import { describe, expect, it } from "vitest";
import {
  createClient,
  WasmChatCompletionRequest,
  WasmResponseFormat,
  WasmChatCompletionTool,
  WasmStreamOptions,
  WasmToolChoice,
  WasmReasoningEffort,
  WasmStopSequence,
  WasmMessage,
  WasmFinishReason,
} from "liter_llm";
function _alefE2eText(value: unknown): string {
  return value == null ? "" : String(value);
}

function _alefE2eItemTexts(item: unknown): string[] {
  if (item == null || typeof item !== "object") {
    return [_alefE2eText(item)];
  }
  const record = item as Record<string, unknown>;
  const itemsText = Array.isArray(record.items) ? record.items.map(_alefE2eText).join(" ") : "";
  return [
    _alefE2eText(item),
    _alefE2eText(record.kind),
    _alefE2eText(record.name),
    _alefE2eText(record.source),
    _alefE2eText(record.alias),
    _alefE2eText(record.text),
    _alefE2eText(record.signature),
    itemsText,
  ];
}

describe("chat", () => {
  it("developer_message: Chat request that includes a developer role message alongside user messages", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/developer_message`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "You are a coding assistant. Always respond with concise code examples.";
            _u.role = "developer";
            return _u;
          })(),
          (() => {
            const _u = new WasmMessage();
            _u.content = "How do I reverse a string in Python?";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        return _u;
      })(),
    );
    expect(result.choices.length).toBe(1);
    expect((result.choices[0].message.content ?? "").trim()).toBe("s[::-1]");
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.Stop);
  });
  it("edge_chat_max_tokens: Chat request with max_tokens=1 terminates with length finish_reason", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/edge_chat_max_tokens`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.maxTokens = 1n;
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "Write a story";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        return _u;
      })(),
    );
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.Length);
    expect((result.choices[0].message.content ?? "").length).toBeGreaterThan(0);
  });
  it("edge_chat_system_only: Chat request with system message and user message", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/edge_chat_system_only`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "You are a helpful and concise assistant";
            _u.role = "system";
            return _u;
          })(),
          (() => {
            const _u = new WasmMessage();
            _u.content = "Hi";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        return _u;
      })(),
    );
    expect((result.choices[0].message.content ?? "").length).toBeGreaterThan(0);
  });
  it("edge_chat_temperature_zero: Chat request with temperature=0 for deterministic responses", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/edge_chat_temperature_zero`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "Say hello";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        _u.temperature = 0;
        return _u;
      })(),
    );
    expect((result.choices[0].message.content ?? "").length).toBeGreaterThan(0);
  });
  it("finish_reason_content_filter: Chat response stopped by content filter with finish_reason of content_filter and null content", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/finish_reason_content_filter`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "Tell me something controversial";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        return _u;
      })(),
    );
    expect(result.choices.length).toBe(1);
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.ContentFilter);
  });
  it("finish_reason_length: Chat response truncated due to max_tokens limit with finish_reason of length", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/finish_reason_length`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.maxTokens = 5n;
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "Tell me a long story";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        return _u;
      })(),
    );
    expect(result.choices.length).toBe(1);
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.Length);
  });
  it("multi_turn_conversation: Multi-turn conversation with system, user, assistant, and follow-up user messages", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/multi_turn_conversation`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "You are a helpful assistant.";
            _u.role = "system";
            return _u;
          })(),
          (() => {
            const _u = new WasmMessage();
            _u.content = "What is 2 + 2?";
            _u.role = "user";
            return _u;
          })(),
          (() => {
            const _u = new WasmMessage();
            _u.content = "2 + 2 equals 4.";
            _u.role = "assistant";
            return _u;
          })(),
          (() => {
            const _u = new WasmMessage();
            _u.content = "And what is 4 + 4?";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        return _u;
      })(),
    );
    expect(result.choices.length).toBe(1);
    expect((result.choices[0].message.content ?? "").trim()).toBe("4 + 4 equals 8.");
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.Stop);
  });
  it("parallel_tool_calls: Chat request that results in parallel tool calls in the response", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/parallel_tool_calls`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "What is the weather in NYC and London?";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        _u.parallelToolCalls = true;
        _u.tools = [
          (() => {
            const _u = new WasmChatCompletionTool();
            _u.function = (() => {
              const _u = new WasmFunctionDefinition();
              _u.description = "Get the current weather for a given location";
              _u.name = "get_weather";
              _u.parameters = {
                properties: { location: { description: "The city name", type: "string" } },
                required: ["location"],
                type: "object",
              };
              return _u;
            })();
            _u.type = "function";
            return _u;
          })(),
        ];
        return _u;
      })(),
    );
    expect(result.choices.length).toBe(1);
    expect((result.choices[0].message.toolCalls ?? "").length).toBeGreaterThan(0);
    expect(result.choices[0].message.toolCalls.length).toBe(2);
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.ToolCalls);
  });
  it("response_format_json_object: Chat request with response_format json_object that returns valid JSON content", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/response_format_json_object`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "Respond with JSON only.";
            _u.role = "system";
            return _u;
          })(),
          (() => {
            const _u = new WasmMessage();
            _u.content = "Give me a user object with name and age fields.";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        _u.responseFormat = (() => {
          const _u = new WasmResponseFormat();
          _u.type = "json_object";
          return _u;
        })();
        return _u;
      })(),
    );
    expect(result.choices.length).toBe(1);
    expect((result.choices[0].message.content ?? "").length).toBeGreaterThan(0);
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.Stop);
  });
  it("response_format_json_schema: Chat request with response_format json_schema that validates the output structure", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/response_format_json_schema`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "What is the temperature in Paris today?";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        _u.responseFormat = (() => {
          const _u = new WasmResponseFormat();
          _u.jsonSchema = {
            name: "weather",
            schema: {
              properties: { temp: { type: "number" } },
              required: ["temp"],
              type: "object",
            },
          };
          _u.type = "json_schema";
          return _u;
        })();
        return _u;
      })(),
    );
    expect(result.choices.length).toBe(1);
    expect((result.choices[0].message.content ?? "").length).toBeGreaterThan(0);
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.Stop);
  });
  it("seed_parameter: Chat request with seed parameter for deterministic output; response includes system_fingerprint", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/seed_parameter`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "Pick a random number";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        _u.seed = 42n;
        return _u;
      })(),
    );
    expect(result.choices.length).toBe(1);
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.Stop);
    expect((result.systemFingerprint ?? "").length).toBeGreaterThan(0);
  });
  it("stop_sequences: Chat request with custom stop sequences that terminates generation at a stop token", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/stop_sequences`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "List items until you see STOP";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        _u.stop = ["STOP", "END"];
        return _u;
      })(),
    );
    expect(result.choices.length).toBe(1);
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.Stop);
  });
  it("tool_choice_required: Chat request with tool_choice set to required forces the model to call a tool", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/tool_choice_required`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "What is the weather today?";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        _u.toolChoice = "required";
        _u.tools = [
          (() => {
            const _u = new WasmChatCompletionTool();
            _u.function = (() => {
              const _u = new WasmFunctionDefinition();
              _u.description = "Get the current weather for a given location";
              _u.name = "get_weather";
              _u.parameters = {
                properties: { location: { description: "The city name", type: "string" } },
                required: ["location"],
                type: "object",
              };
              return _u;
            })();
            _u.type = "function";
            return _u;
          })(),
        ];
        return _u;
      })(),
    );
    expect(result.choices.length).toBe(1);
    expect((result.choices[0].message.toolCalls ?? "").length).toBeGreaterThan(0);
    expect(result.choices[0].message.toolCalls[0].function.name.trim()).toBe("get_weather");
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.ToolCalls);
  });
  it("tool_choice_specific: Chat request with tool_choice specifying a particular function to call", async () => {
    const client = createClient(
      "test-key",
      `${process.env.MOCK_SERVER_URL}/fixtures/tool_choice_specific`,
    );
    const result = await client.chat(
      (() => {
        const _u = new WasmChatCompletionRequest();
        _u.messages = [
          (() => {
            const _u = new WasmMessage();
            _u.content = "What is the weather in Paris?";
            _u.role = "user";
            return _u;
          })(),
        ];
        _u.model = "gpt-4";
        _u.toolChoice = (() => {
          const _u = new WasmToolChoice();
          _u.function = { name: "get_weather" };
          _u.type = "function";
          return _u;
        })();
        _u.tools = [
          (() => {
            const _u = new WasmChatCompletionTool();
            _u.function = (() => {
              const _u = new WasmFunctionDefinition();
              _u.description = "Get the current weather for a given location";
              _u.name = "get_weather";
              _u.parameters = {
                properties: { location: { description: "The city name", type: "string" } },
                required: ["location"],
                type: "object",
              };
              return _u;
            })();
            _u.type = "function";
            return _u;
          })(),
          (() => {
            const _u = new WasmChatCompletionTool();
            _u.function = (() => {
              const _u = new WasmFunctionDefinition();
              _u.description = "Search the web for information";
              _u.name = "search_web";
              _u.parameters = {
                properties: { query: { description: "The search query", type: "string" } },
                required: ["query"],
                type: "object",
              };
              return _u;
            })();
            _u.type = "function";
            return _u;
          })(),
        ];
        return _u;
      })(),
    );
    expect(result.choices.length).toBe(1);
    expect((result.choices[0].message.toolCalls ?? "").length).toBeGreaterThan(0);
    expect(result.choices[0].message.toolCalls[0].function.name.trim()).toBe("get_weather");
    expect(result.choices[0].finishReason).toBe(WasmFinishReason.ToolCalls);
  });
});
