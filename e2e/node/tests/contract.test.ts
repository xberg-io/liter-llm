...
// alef:hash:cff77b159ce0aebef4f0224037ba29c619a0fe4662203cc907ece467f51eaecf
...
describe("contract", () => {
  it("binding_api_parity: Verify all bindings expose the full API surface — constructor accepts all config options and every method exists", async () => {
    ...
    const result = await client.chat({
      messages: [{ content: "Contract test", kind: "user" }],
      model: "openai/gpt-4o",
    });
  ...
  it("contract_ocr: Verify ocr() method exists in all bindings", async () => {
    ...
    await client.ocr({
      document: { kind: "document_url", url: "https://example.com/contract-test.pdf" },
