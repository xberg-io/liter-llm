...
// alef:hash:e607ae065a7ebb34519f94690ad27eb5b9a9fff4d03fc770c2e81851506d3022
...
describe("ocr", () => {
  it("ocr_error_400: 400 Bad Request error when OCR input has an invalid image format", async () => {
    ...
    await expect(async () => {
      await client.ocr({
        document: { kind: "document_url", url: "invalid://url" },
  ...
  it("ocr_error_401: 401 Unauthorized error on OCR request due to invalid API credentials", async () => {
    ...
    await expect(async () => {
      await client.ocr({
        document: { kind: "document_url", url: "https://example.com/doc.pdf" },
        model: "mistral/mistral-ocr-latest",
      });
    }).rejects.toThrow();
  });
  it("ocr_multi_page: OCR request returning multiple pages of document content", async () => {
    ...
    const result = await client.ocr({
      document: { kind: "document_url", url: "https://example.com/multipage.pdf" },
      model: "mistral/mistral-ocr-latest",
    });
