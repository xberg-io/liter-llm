...
// alef:hash:46fcabad68baa1d2c806ea8e2d0e1d74b211b76449e4bf275f2cb10dcf471fc3
...
describe("error-handling", () => {
  ...
  it("auth_401: 401 Unauthorized error when API key is invalid or missing", async () => {
    ...
    await expect(async () => {
      await client.chat({ messages: [{ content: "Hello", kind: "user" }], model: "gpt-4" });
    }).rejects.toThrow();
  });
  it("azure_error_auth: Azure OpenAI returns a 401 Unauthorized error when the API key is missing or invalid — uses Azure's error envelope shape with code AccessDenied", async () => {
    ...
    await expect(async () => {
      await client.chat({ messages: [{ content: "Hello", kind: "user" }], model: "azure/gpt-4" });
    }).rejects.toThrow();
  });
  ...
  it("content_policy_violation: 400 error when a request is rejected due to content policy", async () => {
    ...
    await expect(async () => {
      await client.chat({
        messages: [{ content: "Generate harmful content", kind: "user" }],
        model: "gpt-4",
      });
    }).rejects.toThrow();
  });
  it("context_window_exceeded: 400 error when the prompt exceeds the model's maximum context length", async () => {
    ...
    await expect(async () => {
      await client.chat({
        messages: [
          { content: "Very long prompt that exceeds the context window...", kind: "user" },
        ],
        model: "gpt-4",
      });
    }).rejects.toThrow();
  ...
  it("not_found_404: 404 Not Found error when requesting a model that does not exist", async () => {
    ...
    await expect(async () => {
      await client.chat({ messages: [{ content: "Hello", kind: "user" }], model: "gpt-99" });
    }).rejects.toThrow();
  });
