...
// alef:hash:e5ed93b4c0f78d664d5e43e53080255b9661cf3e5db57dced8d3bbed8df959df
...
final class TypesTests: XCTestCase {
    func testAllMessageTypes() async throws {
        ...
        let _client = try DefaultClient(apiKey: "test-key", baseUrl: AlefE2EMockServer.baseURL + "/fixtures/all_message_types")
    ...
    func testMultimodalContent() async throws {
        ...
        let _client = try DefaultClient(apiKey: "test-key", baseUrl: AlefE2EMockServer.baseURL + "/fixtures/multimodal_content")
        let result = try await _client.chat(_request)
        XCTAssertEqual(result.choices.count, 1)
        XCTAssertEqual(((result.choices[0].finishReason)?.rawValue ?? "").trimmingCharacters(in: CharacterSet.whitespacesAndNewlines), "stop")
    }
