// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:99c189036c5a640d2b8149d2c6cff47a354a439d40bc2daf5bdf24fe3b0f07ab
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
// Issues & docs: https://github.com/kreuzberg-dev/alef
// ignore_for_file: unused_local_variable

import 'package:test/test.dart';
import 'dart:io';
import 'package:liter_llm/liter_llm.dart';
import 'package:liter_llm/src/liter_llm_bridge_generated/frb_generated.dart' show RustLib;
import 'dart:async';
import 'dart:convert';

// E2e tests for category: rerank

String _alefE2eText(Object? value) {
  if (value == null) return '';
  // Check if it's an enum by examining its toString representation.
  final str = value.toString();
  if (str.contains('.')) {
    // Enum.toString() returns 'EnumName.variantName'. Extract the variant name.
    final parts = str.split('.');
    if (parts.length == 2) {
      final variantName = parts[1];
      // Convert camelCase variant names to snake_case for serde compatibility.
      // E.g. 'toolCalls' -> 'tool_calls', 'stop' -> 'stop'.
      return _camelToSnake(variantName);
    }
  }
  return str;
}

String _camelToSnake(String camel) {
  final buffer = StringBuffer();
  for (int i = 0; i < camel.length; i++) {
    final char = camel[i];
    if (char.contains(RegExp(r'[A-Z]'))) {
      if (i > 0) buffer.write('_');
      buffer.write(char.toLowerCase());
    } else {
      buffer.write(char);
    }
  }
  return buffer.toString();
}

Process? _mockServer;
String? _spawnedMockServerUrl;

String _mockServerUrl() => _spawnedMockServerUrl ?? Platform.environment['MOCK_SERVER_URL'] ?? 'http://localhost:8080';

void main() {
  setUpAll(() async {
    await RustLib.init();
    if (Platform.environment['MOCK_SERVER_URL'] == null) {
      final _bin = Platform.script.resolve('../rust/target/release/mock-server').toFilePath();
      final _fixturesDir = Platform.script.resolve('../../fixtures').toFilePath();
      if (File(_bin).existsSync()) {
        _mockServer = await Process.start(_bin, [_fixturesDir], mode: ProcessStartMode.normal);
        final _ready = Completer<void>();
        _mockServer!.stdout
            .transform(utf8.decoder)
            .transform(const LineSplitter())
            .listen((_line) {
          final _trimmed = _line.trim();
          if (_trimmed.startsWith('MOCK_SERVER_URL=')) {
            _spawnedMockServerUrl = _trimmed.substring('MOCK_SERVER_URL='.length);
          } else if (_trimmed.startsWith('MOCK_SERVERS=')) {
            if (!_ready.isCompleted) _ready.complete();
          } else if (_spawnedMockServerUrl != null) {
            if (!_ready.isCompleted) _ready.complete();
          }
        }, onDone: () {
          if (!_ready.isCompleted) _ready.complete();
        });
        await _ready.future.timeout(const Duration(seconds: 30), onTimeout: () {});
      }
    }
  });

  tearDownAll(() async {
    final server = _mockServer;
    if (server != null) {
      server.kill();
      await server.exitCode;
    }
  });

  test('Reranking with an empty query string', () async {
    final _request = await createRerankRequestFromJson(json: '{"documents":["Some document","Another document"],"model":"rerank-v3.5","query":""}');
    final _mockUrl = "${_mockServerUrl()}/fixtures/edge_rerank_empty_query";
        final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
    final result = await _client.rerank(req: _request);
    expect(result.results.length, equals(2));
  });

  test('Reranking with only a single document', () async {
    final _request = await createRerankRequestFromJson(json: '{"documents":["Artificial intelligence is the simulation of human intelligence."],"model":"rerank-v3.5","query":"What is AI?"}');
    final _mockUrl = "${_mockServerUrl()}/fixtures/edge_rerank_single_doc";
        final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
    final result = await _client.rerank(req: _request);
    expect(result.results.length, equals(1));
    expect(result.results[0].relevanceScore, greaterThan(0.9));
  });

  test('401 Unauthorized for reranking with invalid API key', () async {
    await expectLater(() async {
      final _request = await createRerankRequestFromJson(json: '{"documents":["doc1"],"model":"rerank-v3.5","query":"test"}');
      final _mockUrl = "${_mockServerUrl()}/fixtures/error_rerank_auth_401";
          final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
      return _client.rerank(req: _request);
    }(), throwsA(anything));
  });

  test('400 Bad Request for reranking with invalid model', () async {
    await expectLater(() async {
      final _request = await createRerankRequestFromJson(json: '{"documents":["doc1"],"model":"nonexistent-rerank","query":"test"}');
      final _mockUrl = "${_mockServerUrl()}/fixtures/error_rerank_bad_request";
          final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
      return _client.rerank(req: _request);
    }(), throwsA(anything));
  });

  test('Basic reranking of documents against a query', () async {
    final _request = await createRerankRequestFromJson(json: '{"documents":["Machine learning is a subset of AI.","The weather is sunny today.","Deep learning uses neural networks."],"model":"rerank-v3.5","query":"What is machine learning?"}');
    final _mockUrl = "${_mockServerUrl()}/fixtures/smoke_rerank_basic";
        final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
    final result = await _client.rerank(req: _request);
    expect(result.results.length, equals(3));
    expect(result.results[0].relevanceScore, greaterThan(0.9));
  });

  test('Reranking with return_documents flag to include document text', () async {
    final _request = await createRerankRequestFromJson(json: '{"documents":["Rust is a systems programming language.","Iron rusts when exposed to water."],"model":"rerank-v3.5","query":"What is Rust?","return_documents":true}');
    final _mockUrl = "${_mockServerUrl()}/fixtures/smoke_rerank_return_docs";
        final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
    final result = await _client.rerank(req: _request);
    expect(result.results.length, equals(2));
    expect(result.results[0].document, isNotNull);
    expect(result.results[0].relevanceScore, greaterThan(0.9));
  });

  test('Reranking with top_n parameter to limit results', () async {
    final _request = await createRerankRequestFromJson(json: '{"documents":["Python is a programming language.","Cats are cute animals.","Python was created by Guido van Rossum.","The sun is a star."],"model":"rerank-v3.5","query":"What is Python?","top_n":2}');
    final _mockUrl = "${_mockServerUrl()}/fixtures/smoke_rerank_with_top_n";
        final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
    final result = await _client.rerank(req: _request);
    expect(result.results.length, equals(2));
    expect(result.results[0].relevanceScore, greaterThan(0.9));
  });

}
