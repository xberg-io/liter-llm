...
void main() {
  ...
  test('Response completes with empty output items', () async {
    ...
    final _mockUrl = "${_sutUrl()}/fixtures/edge_response_empty_output";
  ...
  test('Response created with a very large input text', () async {
    ...
    final _mockUrl = "${_sutUrl()}/fixtures/edge_response_large_input";
        final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
    final result = await _client.createResponse(req: _request);
    expect(result.id, isNotNull);
    expect(result.status.toString().trim(), equals('completed'.toString().trim()));
  ...
  test('401 Unauthorized when creating a response with invalid API key', () async {
    await expectLater(() async {
      ...
      final _mockUrl = "${_sutUrl()}/fixtures/error_response_auth_401";
          final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
  ...
  test('400 Bad Request when creating response with invalid model', () async {
    await expectLater(() async {
      ...
      final _mockUrl = "${_sutUrl()}/fixtures/error_response_bad_request";
          final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
      return _client.createResponse(req: _request);
    }(), throwsA(anything));
  });
  ...
  test('404 Not Found when retrieving a nonexistent response', () async {
    await expectLater(() async {
      final _mockUrl = "${_sutUrl()}/fixtures/error_response_not_found";
          final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
  ...
  test('Cancel an in-progress response', () async {
    final _mockUrl = "${_sutUrl()}/fixtures/smoke_cancel_response";
        final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
  ...
  test('Create a basic response using the Responses API', () async {
    ...
    final _mockUrl = "${_sutUrl()}/fixtures/smoke_create_response";
        final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
    final result = await _client.createResponse(req: _request);
    expect(result.id, isNotNull);
    expect(result.status.toString().trim(), equals('completed'.toString().trim()));
  ...
  test('Response that includes tool call output items', () async {
    ...
    final _mockUrl = "${_sutUrl()}/fixtures/smoke_response_with_tools";
        final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
    final result = await _client.createResponse(req: _request);
    expect(result.id, isNotNull);
    expect(result.status.toString().trim(), equals('completed'.toString().trim()));
  ...
  test('Retrieve a previously created response', () async {
    final _mockUrl = "${_sutUrl()}/fixtures/smoke_retrieve_response";
        final _client = await LiterLlmBridge.createClient('test-key', baseUrl: _mockUrl);
