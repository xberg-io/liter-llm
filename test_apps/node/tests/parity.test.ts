...
describe('parity', () => {  it('api_surface_parity: Canonical API surface definition for cross-binding parity verification', async () => {    const client = createClient('test-key', `${process.env.MOCK_SERVER_URL}/fixtures/api_surface_parity`);    await client.chat({ messages: [{ content: "Hello", role: "user" }], model: "gpt-4o" });
  }, 30000);
});
