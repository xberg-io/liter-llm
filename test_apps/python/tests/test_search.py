# This file is auto-generated by alef — DO NOT EDIT.
# alef:hash:614a9a228c315e2e022431f3529f2dc8acdbb8c9b89123fe631385dfe0d5ad5f
# To regenerate: alef generate
# To verify freshness: alef verify --exit-code

"""E2e tests for category: search."""

import os
import pytest  # noqa: F401
from liter_llm import create_client, SearchRequest
from liter_llm._internal_bindings import SearchRequest


def _alef_e2e_text(value: object) -> str:
    return "" if value is None else str(value)


def _alef_e2e_item_texts(item: object) -> tuple[str, ...]:
    raw_items = getattr(item, "items", None)
    items_text = " ".join(str(value) for value in raw_items) if isinstance(raw_items, list) else ""
    return (
        _alef_e2e_text(item),
        _alef_e2e_text(getattr(item, "kind", None)),
        _alef_e2e_text(getattr(item, "name", None)),
        _alef_e2e_text(getattr(item, "source", None)),
        _alef_e2e_text(getattr(item, "alias", None)),
        _alef_e2e_text(getattr(item, "text", None)),
        _alef_e2e_text(getattr(item, "signature", None)),
        items_text,
    )


@pytest.mark.asyncio
async def test_search_basic() -> None:
    """Basic web search request with a simple query."""
    client = create_client(api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/search_basic")
    request = SearchRequest.from_json('{"model":"brave/web-search","query":"What is Rust programming language?"}')

    _ = await client.search(request)


@pytest.mark.asyncio
async def test_search_empty_results() -> None:
    """Web search with a query that returns no results."""
    client = create_client(
        api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/search_empty_results"
    )
    request = SearchRequest.from_json('{"model":"brave/web-search","query":"xyznonexistent12345xyz"}')

    _ = await client.search(request)


@pytest.mark.asyncio
async def test_search_error_400() -> None:
    """400 Bad Request error when search query is empty."""
    with pytest.raises(Exception) as exc_info:  # noqa: B017
        client = create_client(
            api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/search_error_400"
        )
        request = SearchRequest.from_json('{"model":"brave/web-search","query":""}')
        await client.search(request)
    assert "BadRequest" in str(exc_info.value) or "BadRequest" in type(exc_info.value).__name__  # noqa: S101


@pytest.mark.asyncio
async def test_search_error_401() -> None:
    """401 Unauthorized error on web search due to invalid API credentials."""
    with pytest.raises(Exception) as exc_info:  # noqa: B017
        client = create_client(
            api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/search_error_401"
        )
        request = SearchRequest.from_json('{"model":"brave/web-search","query":"test"}')
        await client.search(request)
    assert "Authentication" in str(exc_info.value) or "Authentication" in type(exc_info.value).__name__  # noqa: S101


@pytest.mark.asyncio
async def test_search_with_max_results() -> None:
    """Search request with max_results parameter limiting response count."""
    client = create_client(
        api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/search_with_max_results"
    )
    request = SearchRequest.from_json('{"max_results":2,"model":"brave/web-search","query":"Rust programming"}')

    result = await client.search(request)
    assert len(result.results) == 2  # noqa: S101
