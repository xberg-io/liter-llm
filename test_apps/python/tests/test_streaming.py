...
async def test_edge_stream_function_call() -> None:
    ...
    client = create_client(
        api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/edge_stream_function_call"
    )
    request = ChatCompletionRequest.from_json(
...
async def test_stream_content_policy_error() -> None:
    ...
    with pytest.raises(Exception) as exc_info:  # noqa: B017
        client = create_client(
            api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/stream_content_policy_error"
        )
        request = ChatCompletionRequest.from_json(
...
async def test_stream_error_401() -> None:
    ...
    with pytest.raises(Exception) as exc_info:  # noqa: B017
        client = create_client(
            api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/stream_error_401"
        )
        request = ChatCompletionRequest.from_json(
...
async def test_stream_multiple_choices() -> None:
    ...
    client = create_client(
        api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/stream_multiple_choices"
    )
    request = ChatCompletionRequest.from_json(
...
async def test_stream_with_tool_calls() -> None:
    ...
    client = create_client(
        api_key="test-key", base_url=os.environ["MOCK_SERVER_URL"] + "/fixtures/stream_with_tool_calls"
    )
    request = ChatCompletionRequest.from_json(
    ...
    assert (
        str(chunks[-1].choices[0].finish_reason) if chunks and chunks[-1].choices else None
    ).strip() == "tool_calls".strip()  # noqa: S101
    ...
    assert ([t for c in chunks for ch in (c.choices or []) for t in (ch.delta.tool_calls or [])])[
        0
    ].function.name.strip() == "get_weather".strip()  # noqa: S101
