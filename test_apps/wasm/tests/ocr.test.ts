...
describe("ocr", () => {
    ...
    await expect(async () => {
      await client.ocr(
        (() => {
          ...
          _u0.document = { type: "document_url", url: "invalid://url" };
  ...
  it("ocr_error_401: 401 Unauthorized error on OCR request due to invalid API credentials", async () => {
    ...
    await expect(async () => {
      await client.ocr(
        (() => {
          const _u0 = WasmOcrRequest.default();
          _u0.document = { type: "document_url", url: "https://example.com/doc.pdf" };
          _u0.model = "mistral/mistral-ocr-latest";
          return _u0;
        })(),
      );
  ...
  it("ocr_multi_page: OCR request returning multiple pages of document content", async () => {
    ...
    const result = await client.ocr(
      (() => {
        const _u0 = WasmOcrRequest.default();
        _u0.document = { type: "document_url", url: "https://example.com/multipage.pdf" };
        _u0.model = "mistral/mistral-ocr-latest";
        return _u0;
      })(),
    );
