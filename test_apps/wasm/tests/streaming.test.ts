...
describe("streaming", () => {
  it("anthropic_stream: Streaming chat completion via the Anthropic provider (claude-3-5-sonnet-20241022) yielding multiple SSE chunks", async () => {
    ...
    const stream = await client.chatStream(
      (() => {
        ...
        _u0.maxTokens = 32n;
        _u0.messages = [{ content: "Count to three, one word per response.", role: "user" }];
  ...
  it("azure_stream: Streaming chat completion via Azure OpenAI — verifies the azure/ prefix routes correctly and SSE chunks are delivered in the standard OpenAI chat.completion.chunk shape", async () => {
    ...
    const stream = await client.chatStream(
      (() => {
        const _u0 = WasmChatCompletionRequest.default();
        _u0.messages = [{ content: "Count to 3", role: "user" }];
  ...
  it("edge_stream_function_call: Streaming chat completion with tool/function call chunks", async () => {
    ...
    const stream = await client.chatStream(
      (() => {
        const _u0 = WasmChatCompletionRequest.default();
        _u0.messages = [{ content: "What's the weather?", role: "user" }];
        _u0.model = "gpt-4";
        _u0.tools = [
          (() => {
            const _u1 = WasmChatCompletionTool.default();
            _u1.function = (() => {
              const _u2 = WasmFunctionDefinition.default();
              _u2.name = "get_weather";
              _u2.parameters = { properties: { city: { type: "string" } }, type: "object" };
              return _u2;
            })();
            _u1.type = "function";
            return _u1;
  ...
  it("empty_stream: Streaming chat completion that produces no content chunks before the DONE signal", async () => {
    ...
    const stream = await client.chatStream(
      (() => {
        const _u0 = WasmChatCompletionRequest.default();
        _u0.messages = [{ content: "Say nothing", role: "user" }];
        _u0.model = "gpt-4";
        _u0.stream = true;
        return _u0;
      })(),
  ...
  it("stream_content_policy_error: 400 Bad Request error on stream due to content policy violation", async () => {
    ...
    await expect(async () => {
      const _stream = await client.chatStream(
        (() => {
          const _u0 = WasmChatCompletionRequest.default();
          _u0.messages = [{ content: "Generate harmful content", role: "user" }];
          _u0.model = "gpt-4o";
          _u0.stream = true;
          return _u0;
        })(),
  ...
  it("stream_done_signal: Verify that the [DONE] sentinel signal properly terminates the stream", async () => {
    ...
    const stream = await client.chatStream(
      (() => {
        const _u0 = WasmChatCompletionRequest.default();
        _u0.messages = [{ content: "Say done", role: "user" }];
        _u0.model = "gpt-4";
        _u0.stream = true;
        return _u0;
      })(),
  ...
  it("stream_error_401: 401 Unauthorized error on stream initiation before any chunks are received", async () => {
    ...
    await expect(async () => {
      const _stream = await client.chatStream(
        (() => {
          const _u0 = WasmChatCompletionRequest.default();
          _u0.messages = [{ content: "Hello", role: "user" }];
          _u0.model = "gpt-4";
          _u0.stream = true;
          return _u0;
        })(),
  ...
  it("stream_with_tool_calls: Streaming chat completion where the assistant responds with a tool call across multiple chunks", async () => {
    ...
    const stream = await client.chatStream(
      (() => {
        const _u0 = WasmChatCompletionRequest.default();
        _u0.messages = [{ content: "What is the weather in NYC?", role: "user" }];
        _u0.model = "gpt-4";
        _u0.stream = true;
        _u0.tools = [
          (() => {
            ...
            _u1.function = (() => {
              const _u2 = WasmFunctionDefinition.default();
              _u2.description = "Get the current weather for a given location";
              _u2.name = "get_weather";
              _u2.parameters = {
                properties: { location: { description: "The city and state, e.g. New York, NY", type: "string" } },
                required: ["location"],
                type: "object",
              };
              return _u2;
