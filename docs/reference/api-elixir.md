...
| `usage`  | `Usage \| nil`          | `nil`   | Token usage (input tokens only; embeddings have zero output tokens).                                                                  |
...
| `description` | `String.t() \| nil` | `nil`   | Human-readable description explaining what the function does.          |
| `parameters`  | `term() \| nil`     | `nil`   | JSON Schema defining the function's parameters.                        |
| `strict`      | `boolean() \| nil`  | `nil`   | If true, enforce strict JSON schema validation for arguments.          |
...
| `image_base64` | `String.t() \| nil` | `nil`   | Base64-encoded image data (if `include_image_base64` was true). |
...
| `images`     | `list(OcrImage) \| nil` | `nil`   | Embedded images extracted from the page (if `include_image_base64` was true). |
...
| `meta`    | `term() \| nil`      | `nil`   | Optional metadata about the reranking operation. |
...
| `document`        | `RerankResultDocument \| nil` | `nil`   | Original document content (if `return_documents` was true).  |
