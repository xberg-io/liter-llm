...
| `usage`  | `Optional<Usage>`       | `null`  | Token usage (input tokens only; embeddings have zero output tokens).                                                                  |
...
| `description` | `Optional<String>`  | `null`  | Human-readable description explaining what the function does.          |
| `parameters`  | `Optional<Object>`  | `null`  | JSON Schema defining the function's parameters.                        |
| `strict`      | `Optional<Boolean>` | `null`  | If true, enforce strict JSON schema validation for arguments.          |
...
| `imageBase64` | `Optional<String>` | `null`  | Base64-encoded image data (if `include_image_base64` was true). |
...
| `images`     | `Optional<List<OcrImage>>` | `null`  | Embedded images extracted from the page (if `include_image_base64` was true). |
...
| `meta`    | `Optional<Object>`   | `null`  | Optional metadata about the reranking operation. |
...
| `document`       | `Optional<RerankResultDocument>` | `null`  | Original document content (if `return_documents` was true).  |
