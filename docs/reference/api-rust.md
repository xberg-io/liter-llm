...
| `usage`  | `Option<Usage>`        | `None`  | Token usage (input tokens only; embeddings have zero output tokens).                                                                  |
...
| `description` | `Option<String>`            | `None`  | Human-readable description explaining what the function does.          |
| `parameters`  | `Option<serde_json::Value>` | `None`  | JSON Schema defining the function's parameters.                        |
| `strict`      | `Option<bool>`              | `None`  | If true, enforce strict JSON schema validation for arguments.          |
...
| `image_base64` | `Option<String>` | `None`  | Base64-encoded image data (if `include_image_base64` was true). |
...
| `images`     | `Option<Vec<OcrImage>>`  | `None`  | Embedded images extracted from the page (if `include_image_base64` was true). |
...
| `meta`    | `Option<serde_json::Value>` | `None`  | Optional metadata about the reranking operation. |
...
| `document`        | `Option<RerankResultDocument>` | `None`  | Original document content (if `return_documents` was true).  |
