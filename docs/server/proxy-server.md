...
The `liter-llm` binary ships with a production proxy that speaks the OpenAI REST API on top of the 142 runtime providers. It terminates Bearer auth, routes by model name, applies the full Tower middleware stack (cache, budget, rate limit, cooldown, health, fallback), and exposes OpenTelemetry spans.
