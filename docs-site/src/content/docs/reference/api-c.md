...
```c
LiterllmModelInfo* result = literllm_model_info("value");
```
...
```c
void literllm_install_catalog_overlay_from_str(const char* catalog_json);
```
...
```c
LiterllmRefreshOutcome* literllm_refresh_catalog(LiterllmCatalogRefreshConfig config);
```

**Example:**

```c
LiterllmRefreshOutcome *result = literllm_refresh_catalog(NULL);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `config` | `LiterllmCatalogRefreshConfig` | Yes | The configuration options |

**Returns:** `LiterllmRefreshOutcome`
...
```c
const char** literllm_reasoning_text();
```

**Example:**

```c
const char** result = literllm_reasoning_text(instance);
```
...
| `source_url` | `const char*` | — | Source URL to fetch `catalog.json` from. Must be `https`. Defaults to `DEFAULT_CATALOG_URL`; configurable so self-hosted mirrors work. |
...
```c
LiterllmCatalogRefreshConfig literllm_default();
```

**Example:**

```c
LiterllmCatalogRefreshConfig *result = literllm_default();
```

**Returns:** `LiterllmCatalogRefreshConfig`
...
| `tiers` | `LiterllmModelTier*` | `NULL` | Context-tiered pricing overrides, sorted by ascending `min_context_tokens`. Empty when the model has flat pricing. |
