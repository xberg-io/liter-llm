...
## Zig API Reference <span class="version-badge">v1.10.1</span>
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `catalogJson` | `\[:0\]const u8` | Yes | The catalog json |
...
```zig
const result = try refreshCatalog(.{});
```
...
| `reasoningContent` | `\[:0\]const u8?` | `null` | Reasoning/thinking tokens returned by the provider, if any (e.g. DeepSeek R1, Qwen `reasoning_content`, or Anthropic extended thinking). |
...
```zig
pub fn reasoningText(self: *const AssistantMessage) ?[:0]const u8
```

**Example:**

```zig
const result = instance.reasoningText();
```
...
| `sourceUrl` | `\[:0\]const u8` | — | Source URL to fetch `catalog.json` from. Must be `https`. Defaults to `DEFAULT_CATALOG_URL`; configurable so self-hosted mirrors work. |
| `ttlSeconds` | `u64` | `86400` | How long a cached `catalog.json` remains valid before a network refetch is attempted, in seconds. |
| `cachePath` | `\[:0\]const u8?` | `null` | Filesystem path for the on-disk cache. `null` uses a default path under `std.env.temp_dir()`. |
...
```zig
const result = CatalogRefreshConfig.default();
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `inputCostPerToken` | `f64` | — | Cost in USD per input (prompt) token. |
| `outputCostPerToken` | `f64` | — | Cost in USD per output (completion) token. |
| `cacheReadInputTokenCost` | `f64?` | `null` | Cost in USD per cached input token (cache hit / read). |
| `cacheCreationInputTokenCost` | `f64?` | `null` | Cost in USD per token written to the prompt cache. |
| `inputCostPerAudioToken` | `f64?` | `null` | Cost in USD per input audio token. |
| `outputCostPerAudioToken` | `f64?` | `null` | Cost in USD per output audio token. |
| `outputCostPerReasoningToken` | `f64?` | `null` | Cost in USD per reasoning (extended-thinking) output token. |
| `maxTokens` | `u64?` | `null` | Total context window size in tokens (input + output). |
| `maxInputTokens` | `u64?` | `null` | Maximum input (prompt) tokens accepted. |
| `maxOutputTokens` | `u64?` | `null` | Maximum output (completion) tokens the model can generate. |
...
| `ownedBy` | `\[:0\]const u8` | — | Organization or entity that owns the model. Defaults to empty when a provider omits the field. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `minContextTokens` | `u64` | — | The tier applies when the prompt/context token count is at least this value. |
| `inputCostPerToken` | `f64` | — | Cost in USD per input (prompt) token within this tier. |
| `outputCostPerToken` | `f64` | — | Cost in USD per output (completion) token within this tier. |
| `cacheReadInputTokenCost` | `f64?` | `null` | Cost in USD per cached input token within this tier. |
| `cacheCreationInputTokenCost` | `f64?` | `null` | Cost in USD per cache-write token within this tier. |
| `inputCostPerAudioToken` | `f64?` | `null` | Cost in USD per input audio token within this tier. |
| `outputCostPerAudioToken` | `f64?` | `null` | Cost in USD per output audio token within this tier. |
| `outputCostPerReasoningToken` | `f64?` | `null` | Cost in USD per reasoning output token within this tier. |
...
| `reasoningContent` | `\[:0\]const u8?` | `null` | Partial reasoning/thinking tokens (OpenAI-compatible extension used by DeepSeek R1, Qwen, etc.). |
