...
The liter-llm MCP server exposes 22 tools for unified access to 163 runtime LLM providers, embeddings, files, batches, and more. Integrate it with VS Code, GitHub Copilot, Claude Desktop, Cursor, and other MCP-compatible IDEs and applications.
...
Instead of hard-coding integrations for each provider (OpenAI, Anthropic, Google, Groq, etc.), MCP lets your IDE or application call any of the 163 runtime providers via a single interface.
...
- [Provider Registry](/providers/) — Browse all 163 runtime providers
