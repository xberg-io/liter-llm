...
import { Content as Snip_curl_server_chat } from "../../../snippets/curl/server/chat.md";

The `liter-llm` binary ships with a production proxy that speaks the OpenAI REST API on top of the 163 runtime providers. It terminates Bearer auth, routes by model name, applies the full Tower middleware stack (cache, budget, rate limit, cooldown, health, fallback), and exposes OpenTelemetry spans.
...
<Snip_curl_server_chat />

See [Proxy Configuration](/server/proxy-configuration/) for every config field.
