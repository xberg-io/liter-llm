...
- `iter26/wrap`: skip `edge_file_large_upload` for wasm — reqwest's `wasm32` multipart backend builds the body via `web_sys::FormData` + Blob and the resulting POST is rejected at fetch time (`error sending request`) in the vitest/Node environment before reaching the mock server. The 13 other backends use native reqwest multipart and pass the same fixture. Documented the Xcode 26.x / Swift 6.x SDK ↔ toolchain alignment requirement in `packages/swift/BUILDING.md` (covers the `ld: framework 'SwiftUICore' not found` symptom that appears when a sideloaded toolchain version diverges from the active Xcode SDK).
- `iter26`: bump alef pin to `0.15.56`, regenerate all 14 language bindings + e2e suites. Picks up: kotlin scaffold `srcDir(".")` so the alef-emitted facade ends up in the jar (fixes `Unresolved reference 'LiterLlm'` in e2e), kotlin streaming codegen sticky-nullability through chained method calls and `chunks.last().usage()` rewrite for streaming-virtual fields, go transcribe value-typed element nil-check, swift `not_empty`/`is_empty` over `RustVec<T>` via `.len()` instead of `.toString().isEmpty`, kotlin mock-URL system-property fallback for the in-process MockServerListener, and Java FFM `Arena.ofShared()` so kotlin coroutines (and any thread-hopping caller) don't trip `WrongThreadException`. Fixture skips for `local_chat_ollama`, `local_list_models_ollama`, and `error_file_bad_purpose` extended to include kotlin to match python's platform-divergence rationale.
