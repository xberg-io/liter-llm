...
- `WaitForBatchConfig` — configuration struct with `initial_interval`, `max_interval`, `backoff_multiplier`, and optional `timeout` fields; implements `Default` with 5s initial, 60s max, 1.5x multiplier, no timeout.
- `BatchWaitError` — error enum with variants `Failed(BatchStatus)`, `Timeout(Duration)`, and `Client(LiterLlmError)` for batch polling failures.
