...
- Regenerate all bindings against a locally-built alef with seven codegen fixes (queued for an upstream `0.17.11` release):
  ...
  - **php streaming**: `chat_stream` returns `Vec<String>` (each element a JSON-serialized chunk) via ext-php-rs; the e2e collect snippet now decodes each element AND recursively camelCases the keys so the test accessors (`$c->choices[0]->delta->finishReason`) line up with the camelCase contract the non-streaming PHP binding emits.
  - **wasm**: streaming codegen now emits a `while/next()` drain instead of `for await`. wasm-bindgen does not auto-emit `Symbol.asyncIterator` on the generated `ChatStreamIterator`, so the previous shared NAPI `for await` snippet threw `TypeError: stream is not async iterable` on every streaming test. Both the chunk-collect snippet and the streaming-error drain in `typescript/test_function.jinja` now branch on `lang == "wasm"`.
  - **csharp**: replace `(<param> ?? new <Config>())` fallback at custom-provider registration with `<param> ?? throw new ArgumentNullException(nameof(<param>))`. Generated C# records may declare `required` members (e.g. `CustomProviderConfig.Name/BaseUrl/AuthHeader`), making `new T()` a CS9035 compile error.
  - **csharp Id casing**: remove `"ID"` from `CSHARP_INITIALISMS` so `id` fields become `.Id` (matching modern .NET conventions and what the e2e codegen already emits via `to_upper_camel_case`). Previously the binding emitted `.ID` while tests called `.Id`, causing 50+ `CS1061: 'ResponseObject' does not contain a definition for 'Id'` errors.
  - **rustler (elixir)**: when the core function returns `&[T]` / `&'static [T]` (e.g. `liter_llm::provider::all_providers`), emit `.iter().cloned().map(Into::into).collect()` instead of `.into_iter().map(Into::into).collect()`. `into_iter()` on a slice yields `&T`, which fails the `From<T> for TBinding` impl (3× E0277 in `liter_llm_nif/src/lib.rs:1924`).
  - **pyo3 (python)**: filter out Rust primitive types (`String`, `&str`, `Vec<u8>`, `()`, integers, floats, bool) when collecting adapter param types for import. Previously a stray `from .options import …, String` line was emitted whenever an adapter declared a `String` param, which exploded with `ImportError: cannot import name 'String' from 'liter_llm.options'` at module load.
  - **pyo3 scaffold**: `pyproject.toml` template now ships with `[[tool.mypy.overrides]]` disabling the four error codes triggered by the `options.*`-dataclass / `_internal_bindings.*`-pyclass discrepancy in `api.py`, plus `lint.per-file-ignores` for the cosmetic ruff rules (`F401`/`I001`/`UP035`/`RUF100`) the wrapper modules still emit. Removes the need for downstream maintainers to re-add these overrides after every alef regen.
...
- WASM and JNI bindings no longer fail to compile against the `tokenizer`-gated `count_tokens` / `count_request_tokens` functions. `tokenizers` (HF) pulls in `hf-hub` and `fancy-regex`, neither of which build cleanly for `wasm32-unknown-unknown` without `unstable_wasm`, and the value of HuggingFace tokenization on Android NDK is dubious; both bindings now `exclude_functions = ["count_tokens", "count_request_tokens"]` in `alef.toml`. Apps that need token counting on those targets should call a server-side endpoint.
- C / FFI header now emits the opaque `typedef struct LITERLLMLiterLlmError LITERLLMLiterLlmError;` referenced by the recently added `literllm_liter_llm_error_{status_code,is_transient,error_type}` accessors. Previously the header compiled with `unknown type name 'LITERLLMLiterLlmError'`, breaking every C consumer using the new error-introspection API.
- Go binding multi-line doc comments are now `//`-prefixed on every continuation line. Previously, continuation lines after the first paragraph were emitted bare, producing `expected declaration, found Maps` at `packages/go/binding.go:96` and breaking `go build`.
- Java `ResponseObject` / `ResponseTool` DTOs now round-trip the full OpenAI Responses payload. `ResponseOutputItem.content` is a `List<…>` (was a misaligned `LinkedHashMap`); `ResponseTool` accepts the `description` field via the `@JsonAnyGetter` / `@JsonAnySetter` flatten path. Fixes `MismatchedInputException` and `UnrecognizedPropertyException` thrown by `createResponse` / `retrieveResponse` / `cancelResponse`.
- Node (NAPI) streaming HTTP-init errors (e.g. 400 content-policy, 401 unauthorized on `chatStream`) now reject through the iterator. The binding remains lazy (parity with Python's `async for _ in stream: pass` pattern); the e2e error-assertion tests drain the stream inside the `await expect(...).rejects.toThrow()` block to surface the error.
- PHP binding clippy clean under `-D warnings`: scalar getters on DTO classes preserve their `Option<T>` return type when the original field is `Option<T>` + `#[serde(skip_serializing_if = "Option::is_none")]` (`get_window`, `get_ttl` previously returned bare `T` causing E0308); FFI emits `.unwrap_or(default)` instead of `match Some(val) => val, None => default` (clippy::manual_unwrap_or); slice-to-owned conversions across NAPI, pyo3, FFI, and wasm-bindgen use `.iter().cloned().map(Into::into)` instead of `.into_iter().map(|v| v.clone().into())` (clippy::into_iter_on_ref).
- Python `api.py` wrapper now emits the correct shape for non-streaming methods. Previously every `DefaultClient` method (`chat`, `embed`, `ocr`, file/batch/response ops — 22 in total) was wrapped as `async def foo(...) -> AsyncIterator[()]: async for item in engine.foo(): yield item`, but only `chat_stream` is genuinely streaming. The wrapper now emits `async def foo(engine, ...) -> ReturnType: return await engine.foo(...)` for `async_method` adapters; `chat_stream` retains the streaming form. Also fixes `String` (Rust) → `str` (Python) and `bytes::Bytes` → `bytes` mappings, and imports adapter return types into `api.py` so bare names in signatures resolve.
