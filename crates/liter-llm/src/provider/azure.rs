...
mod tests {
    ...
    fn transform_request_o_series_removes_temperature_and_top_p() {
        ...
        assert_eq!(body.get("reasoning_effort").expect("reasoning_effort should be present"), "high");
        // messages preserved
        assert!(body.get("messages").is_some());
    }
    ...
    fn transform_response_content_filter_blocked_no_message() {
        ...
        assert!(message["refusal"].as_str().expect("refusal should be a string").contains("Content filtered"));
    }
