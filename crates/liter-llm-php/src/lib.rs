...
// alef:hash:52c8c64484c868069879865d8ef64f617b3c5b9a8a18b892f22ffdad16a186d2
...
/// Streaming handle for `DefaultClient::chat_stream` — owns the live `BoxStream` plus any async state.
...
/// Open a streaming `chat_stream` request. Returns an opaque handle (as u64)
...
pub fn defaultclient_chat_stream_start(
    ...
    let resource_obj: DefaultClient = resource
        .clone()
        .try_into()
        .map_err(|e| ext_php_rs::exception::PhpException::default(format!("invalid resource: {}", e)))?;
    let stream = WORKER_RUNTIME
        .block_on(async { resource_obj.inner.chat_stream(core_req).await })
...
pub fn defaultclient_chat_stream_next(
    handle: i64,
) -> std::result::Result<Option<String>, ext_php_rs::exception::PhpException> {
    ...
    let handle_ptr = handle as u64 as *mut DefaultClientChatStreamHandle;
    // SAFETY: caller guarantees `handle_ptr` is valid (returned by defaultclient_chat_stream_start) and
    // not yet freed. The PHP wrapper owns the handle lifecycle.
    let handle_ref = unsafe { &*handle_ptr };
    let mut guard = handle_ref
        .stream
        .lock()
...
#[php_function]
pub fn defaultclient_chat_stream_free(handle: i64) -> std::result::Result<(), ext_php_rs::exception::PhpException> {
    if handle <= 0 {
        return Ok(());
    }
    let handle_ptr = handle as u64 as *mut DefaultClientChatStreamHandle;
...
#[derive(Clone)]
#[php_class]
#[php(name = "Liter\\Llm\\TowerCachedResponse")]
