...
impl LiterLlmApi {
    ...
    pub fn count_request_tokens(model: String, req: &ChatCompletionRequest) -> PhpResult<i64> {
        let req_core: liter_llm::ChatCompletionRequest = req.clone().into();
        let result = liter_llm::count_request_tokens(&model, &req_core)
            .map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(result as i64)
    }
